#!/usr/bin/env python3
"""
V20: 延迟价差套利 (Latency Spread Arbitrage)
...
FEE_LIGHTER_MAKER = Decimal("0")          # free account
...
class SpreadArbBot:
    ...
    def __init__(
        ...
        self.csv_path = f"logs/spread_v20_{self.symbol}_trades.csv"
        self.log_path = f"logs/spread_v20_{self.symbol}.log"
        self._init_csv()
    ...
    def _init_logger(self) -> logging.Logger:
        lg = logging.getLogger(f"v20_{self.symbol}")
    ...
    def _on_lighter_fill(self, od: dict):
        ...
        if (is_our_buy or is_our_sell) and not self._fill_event.is_set():
            ...
            self.logger.info(
                f"[Lighter] 平仓期间成交: {side} {filled_base} @ {avg_price}")
        else:
            self.logger.warning(
                f"[Lighter] 非预期成交(已更新仓位,不触发对冲): "
    ...
    async def _hedge_extended(self, side: str, qty: Decimal, ref_price: Decimal) -> Optional[Decimal]:
        """Place IOC on Extended. Returns fill price or None."""
        slip = Decimal("0.005")
        if side == "buy":
        ...
        self._x_ioc_pending_id = f"pending_{time.time()}"
        ...
        self.logger.info(f"[Extended] IOC已发送({(_t1-_t0)*1000:.0f}ms) id={result.order_id}")

        if self._x_ioc_confirmed.is_set():
            self.logger.info("[Extended] WS已提前确认")
        else:
            try:
                await asyncio.wait_for(self._x_ioc_confirmed.wait(), timeout=0.15)
            except asyncio.TimeoutError:
                pass
        _t2 = time.time()
        self.logger.info(f"[Extended] WS等待={(_t2-_t1)*1000:.0f}ms confirmed={self._x_ioc_confirmed.is_set()}")

        if self._x_ioc_fill_qty > 0 and self._x_ioc_fill_price:
            if side == "buy":
                self.extended_position += self._x_ioc_fill_qty
            else:
                self.extended_position -= self._x_ioc_fill_qty
            return self._x_ioc_fill_price

        # WS 没及时确认，但 IOC HTTP 成功 → 信任已成交
        if side == "buy":
    ...
    async def _close_position(self, l_bid: Decimal, l_ask: Decimal,
        ...
        _fee = float(FEE_EXTENDED_TAKER) * 10000
        ...
        self.logger.info(
            ...
            f"close_cost={_net:.2f}bps  {_ms:.0f}ms  "
            f"累积={self.cumulative_net_bps:+.2f}bps")

        self._trade_seq += 1
        self._csv_row([
            datetime.now(_TZ_CN).isoformat(),
            f"v20_close_{self._trade_seq:04d}",
            f"close_{l_side}",
    ...
    async def run(self):
        ...
        self._bn_ws_task: Optional[asyncio.Task] = None
        ...
        try:
            ...
            # 取消 WS 任务
            for t in [self._lighter_ws_task, self._bn_ws_task]:
                if t and not t.done():
                    t.cancel()
    ...
    async def _main_loop(self):
        ...
        self._bn_ws_task = asyncio.create_task(self.binance_feed.connect())
        ...
        while time.time() - t0 < 20 and not self.stop_flag:
            ...
            if time.time() - t0 > 5 and int(time.time() - t0) % 3 == 0:
                self.logger.info(
                    ...
                    f"BN={'OK' if self.binance_feed.mid > 0 else 'WAIT'}")
            await asyncio.sleep(0.5)
        ...
        self.logger.info(
            ...
            f"X={ext_bid}/{ext_ask}  BN={self.binance_feed.mid:.1f}")
        ...
        while not self.stop_flag:
            try:
                ...
                if now - last_status_log > 15:
                    bn_mid = self.binance_feed.mid if self.binance_feed else 0
                    self.logger.info(
                        f"[{self.state.value}] basis={basis_bps:+.1f}bps  "
                        f"L={l_bid}/{l_ask} X={ext_bid}/{ext_ask} BN={bn_mid:.1f}  "
                ...
                if not self.dry_run and now - last_reconcile > 60 and self.state == State.IDLE:
                    await self._reconcile()
                    last_reconcile = now
                    _imbalance = abs(self.lighter_position + self.extended_position)
                    if _imbalance >= self.extended_min_order_size:
                        self.logger.warning(
                            f"[对账] 仓位不平衡={_imbalance} "
                ...
                # ━━━━━━ STATE: QUOTING ━━━━━━
                    ...
                    if self._fill_event.is_set():
                        ...
                        # 成交了就必须对冲，不管spread够不够
                        # 单腿暴露风险 >> 一笔亏损对冲的成本
                        if _real_spread < 0:
                            self.logger.warning(
                                f"[QUOTING] 成交但spread为负({_real_spread:.1f}bps)! "
                                f"仍然对冲(防单腿)")
                        else:
                            self.logger.info(
                                f"[QUOTING] Lighter {_f_side} 成交! "
                                f"实时spread={_real_spread:.1f}bps → 对冲")
                ...
                # ━━━━━━ STATE: HEDGING ━━━━━━
                    ...
                    if self.bn_confirm and self.binance_feed and self.binance_feed.mid > 0:
                        bn = self.binance_feed.mid
                        if _hedge_side == "buy" and bn > float(_ref) * 1.003:
                            self.logger.warning(f"[HEDGING] BN确认失败: BN={bn} > ref={_ref}*1.003, 仍执行")
                        elif _hedge_side == "sell" and bn < float(_ref) * 0.997:
                            self.logger.warning(f"[HEDGING] BN确认失败: BN={bn} < ref={_ref}*0.997, 仍执行")
                    ...
                    if _hedge_ok and _x_fill_price:
                        ...
                        self.logger.info(
                            ...
                            f"hedge={_hedge_ms:.0f}ms  累积={self.cumulative_net_bps:+.2f}bps")

                        self._trade_seq += 1
                        self._csv_row([
                            datetime.now(_TZ_CN).isoformat(),
                            f"v20_{self._trade_seq:04d}",
                            _side,
...
def main():
    p = argparse.ArgumentParser(description="V20 价差收敛套利")
    ...
    bot = SpreadArbBot(
        ...
        bn_confirm=not args.no_bn_confirm,
        interval=args.interval,
        dry_run=args.dry_run,
    )
    bot.flatten_on_start = args.flatten_on_start

    fee_bps = float(FEE_EXTENDED_TAKER) * 10000
    rt_fee = fee_bps * 2
