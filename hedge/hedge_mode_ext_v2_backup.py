...
class HedgeBot:
    ...
    def compute_signal(self) -> Optional[Tuple[str, Decimal, Decimal]]:
        ...
        if minutes_in_hour >= (60 - self.funding_guard_minutes) or minutes_in_hour < self.funding_guard_minutes:
            return None
        ...
        # Determine closing direction based on current Lighter position
        close_side = None
        ...
        # Compute effective close_buffer with time-decay.
        # Decays from close_buffer towards -open_buffer (max urgency)
        # so the bot eventually closes even against a persistent premium.
            ...
            if rounded > 0:
                ...
                if now - self._last_close_log_time >= 10:
                    ...
                    self.logger.info(
                        ...
                        f"平仓安全距离=${sell_safety:.4f}{hold_str}")
                    self._last_close_log_time = now
                return ('sell', rounded, self.order_quantity)
            ...
            if rounded > 0:
                ...
                if now - self._last_close_log_time >= 10:
                    ...
                    self.logger.info(
                        ...
                        f"平仓安全距离=${buy_safety:.4f}{hold_str}")
                    self._last_close_log_time = now
                return ('buy', rounded, self.order_quantity)

        # Priority 2: opening signals
        best_signal = None
    ...
    async def trading_loop(self):
        ...
        while not self.stop_flag:
            try:
                ...
                # Drain stale fills: close small ones on Lighter, hedge large ones on Extended
                while self._stale_fills and self.state == STATE_IDLE:
                    ...
                    if sf['size'] < self.order_quantity:
                        agg_price = (self.lighter_best_bid - self.tick_size
                                     if hedge_side == 'sell'
                                     else self.lighter_best_ask + self.tick_size)
                        if not agg_price or agg_price <= 0:
                            self.logger.warning(
                                f"[修复] 过期成交 {sf['side']} {sf['size']} "
                                f"无有效价格, 暂存等待下次处理")
                            self._stale_fills.insert(0, sf)
                            break
                        agg_price = self._round_lighter_price(agg_price)
                        self.logger.warning(
                            f"[修复] 过期小额成交 {sf['side']} {sf['size']}, "
                            f"在Lighter直接市价平仓: {hedge_side} {sf['size']} @ {agg_price}")
                        try:
                            is_ask = (hedge_side == 'sell')
                            _, _, error = await self.lighter_client.create_order(
                                market_index=self.lighter_market_index,
                                client_order_index=int(time.time() * 1000),
                                base_amount=int(sf['size'] * self.base_amount_multiplier),
                                price=int(agg_price * self.price_multiplier),
                                is_ask=is_ask,
                                order_type=self.lighter_client.ORDER_TYPE_LIMIT,
                                time_in_force=self.lighter_client.ORDER_TIME_IN_FORCE_GOOD_TILL_TIME,
                                reduce_only=True,
                            ...
                            if error is not None:
                                self.logger.error(f"[修复] Lighter平仓小单出错: {error}")
                            else:
                                self.logger.info(
                                    f"[修复] Lighter平仓小单已发送: {hedge_side} {sf['size']} @ {agg_price}")
                        except Exception as e:
                            self.logger.error(f"[修复] Lighter平仓小单异常: {e}")
                        continue
                    self.logger.warning(
