#!/usr/bin/env python3
"""
V24: 阶梯建仓 + 即时对冲 + 余额校验 (Ladder Entry + Instant Hedge + Balance Verify)

基于V23, 新增:
  9. 阶梯建仓: 价差越大越积极, 分层开仓(--ladder-step bps 每层间距, 最多3层)
     第1层: spread >= open-spread             → 开 size
     第2层: spread >= open-spread + step      → 再开 size
     第3层: spread >= open-spread + 2*step    → 再开 size
  10. 平仓: 与V23一致, 价差收敛到close-spread时一次性全部平仓
...
用法:
  python spread_arb_v24.py --symbol BTC --size 0.001 --open-spread 4 --close-spread 0 --ladder-step 1.5
  python spread_arb_v24.py --symbol BTC --size 0.002 --max-position 0.01 --open-spread 4 --ladder-step 2
"""
...
class SpreadArbBot:
    ...
    def __init__(
        ...
        # ── V24: Ladder level (当前阶梯层级, 每次成交后+1, 平仓后归零) ──
        self._ladder_level: int = 0

        # ── Logging / CSV ──
        os.makedirs("logs", exist_ok=True)
        self.csv_path = f"logs/spread_v24_{self.symbol}_trades.csv"
        self.log_path = f"logs/spread_v24_{self.symbol}.log"
        self._init_csv()
    ...
    def _init_logger(self) -> logging.Logger:
        lg = logging.getLogger(f"v24_{self.symbol}")
    ...
    def compute_maker_prices(
        ...
        V24: 内部价差 = open_spread + ladder_level * ladder_step + spread_buffer
    ...
    async def _close_position(self, l_bid: Decimal, l_ask: Decimal,
        ...
        """平仓: Lighter IOC taker(0费) + Extended IOC taker(2.25bps)。"""
        ...
        self._csv_row([
            ...
            f"v24_close_{self._trade_seq:04d}",
            f"close_{l_side}",
        ...
        if self.ladder_step_bps > 0 and self._ladder_level > 0:
            self.logger.info(f"[阶梯] 平仓完成, L{self._ladder_level} → L0")
    ...
    async def _v23_record_pre_trade_balance(self):
        ...
        try:
            ...
            if _l is not None and _x is not None:
                self.logger.info(
                    f"[V24] 开仓后余额快照: L={_l} X={_x} 合计={_l+_x}")
        except Exception as e:
            self.logger.warning(f"[V24] 开仓余额快照失败: {e}")
    ...
    async def _v23_verify_close_pnl(self, formula_net_bps: float, formula_dollar: float,
        ...
        try:
            ...
            if _pre_l is None or _pre_x is None:
                self.logger.info(
                    f"[V24] 平仓后余额: L={_l_after} X={_x_after} (无开仓基准, 跳过对比)")
            ...
            self.logger.warning(f"[V24] 余额校验异常: {e}")
            return "", "", "", ""
    ...
    async def _main_loop(self):
        ...
        self.logger.info(
            f"启动 V24 阶梯建仓+余额校验  {self.symbol}  "
        ...
        if not self.dry_run:
            ...
            if _l_bal is not None and _x_bal is not None:
                self.logger.info(
                    f"[V24] 初始余额: Lighter={_l_bal}  Extended={_x_bal}  "
                    f"合计={_l_bal + _x_bal}")
            else:
                self.logger.warning(
                    f"[V24] 初始余额部分获取失败: L={_l_bal} X={_x_bal}")
...
def main():
    p = argparse.ArgumentParser(description="V24 阶梯建仓+余额校验")
