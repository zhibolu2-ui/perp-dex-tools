#!/usr/bin/env python3
"""
V32: JIT即时狙击 + 阶梯建仓 + 快速平仓

核心: 只在价差够大时挂maker单在Lighter簿第一位, 最多等2秒
  - 不长时间挂单, 不需要modify, 暴露时间极短
  - spread够 → 挂单(紧贴市场) → 2秒内被吃 → 对冲 → 赚
  - 2秒没被吃 → 撤单 → 继续等下次机会
...
用法:
  python spread_arb_v32.py --symbol BTC --size 0.002 --max-position 0.01 --open-spread 4 --close-spread 0 --ladder-step 1.5
"""
...
class SpreadArbBot:
    ...
    def __init__(
        ...
        self.csv_path = f"logs/spread_v32_{self.symbol}_trades.csv"
        self.log_path = f"logs/spread_v32_{self.symbol}.log"
        self._init_csv()
    ...
    def _init_logger(self) -> logging.Logger:
        lg = logging.getLogger(f"v32_{self.symbol}")
    ...
    async def _close_position(self, l_bid: Decimal, l_ask: Decimal,
        ...
        # V32: JIT maker平仓 — 紧贴市场挂单2秒, 没成交则fallback到taker IOC
        if l_side == "buy":
            _jit_close_price = self._round_lighter(l_bid - self.lighter_tick)
        else:
            _jit_close_price = self._round_lighter(l_ask + self.lighter_tick)
        ...
        try:
            _, _, err = await self.lighter_client.create_order(
                ...
                price=int(_jit_close_price * self.price_multiplier),
                is_ask=(l_side == "sell"),
                order_type=self.lighter_client.ORDER_TYPE_LIMIT,
                time_in_force=self.lighter_client.ORDER_TIME_IN_FORCE_GOOD_TILL_TIME,
                reduce_only=True,
            ...
            if err:
                self.logger.warning(f"[平仓] JIT maker挂单失败: {err}, fallback到taker")
            else:
                self.logger.info(
                    f"[JIT平仓] L_{l_side} {close_qty}@{_jit_close_price} (紧贴市场) → 等2秒")
                try:
                    await asyncio.wait_for(
                        self._flatten_fill_event.wait(), timeout=2.0)
                except asyncio.TimeoutError:
                    pass

                if self._flatten_fill_event.is_set():
                    _l_used_jit = True
                    self.logger.info(f"[JIT平仓] maker成交!")
                else:
                    await self._cancel_all_lighter()
                    await asyncio.sleep(0.05)
                    if self._flatten_fill_event.is_set():
                        _l_used_jit = True
                        self.logger.info(f"[JIT平仓] 撤单时成交!")
                    else:
                        self.logger.info(f"[JIT平仓] 2秒未成交, fallback到taker")
        except Exception as e:
            self.logger.warning(f"[平仓] JIT maker异常: {e}, fallback到taker")
        ...
        if not _l_used_jit:
            ...
            try:
                ...
                if err:
                    self.logger.error(f"[平仓] Lighter taker也失败: {err}")
                    self._in_flatten = False
                    return False
            except Exception as e:
        ...
        self._csv_row([
            ...
            f"v32_close_{self._trade_seq:04d}",
            f"close_{l_side}",
    ...
    async def _v23_record_pre_trade_balance(self):
        ...
        try:
            ...
            if _l is not None and _x is not None:
                self.logger.info(
                    f"[V32] 开仓后余额快照: L={_l} X={_x} 合计={_l+_x}")
        except Exception as e:
            self.logger.warning(f"[V32] 开仓余额快照失败: {e}")
    ...
    async def _v23_verify_close_pnl(self, formula_net_bps: float, formula_dollar: float,
        ...
        try:
            ...
            if _pre_l is None or _pre_x is None:
                self.logger.info(
                    f"[V32] 平仓后余额: L={_l_after} X={_x_after} (无开仓基准, 跳过对比)")
            ...
            self.logger.warning(f"[V32] 余额校验异常: {e}")
            return "", "", "", ""
    ...
    async def _main_loop(self):
        ...
        self.logger.info(
            f"启动 V32 JIT开仓+JIT平仓  {self.symbol}  "
            f"size={self.order_size}  "
            f"JIT: spread≥{self.open_spread_bps}bps(+{self.spread_buffer_bps}bps) → 紧贴挂单2秒  "
        ...
        if not self.dry_run:
            ...
            if _l_bal is not None and _x_bal is not None:
                self.logger.info(
                    f"[V32] 初始余额: Lighter={_l_bal}  Extended={_x_bal}  "
                    f"合计={_l_bal + _x_bal}")
            else:
                self.logger.warning(
                    f"[V32] 初始余额部分获取失败: L={_l_bal} X={_x_bal}")
        ...
        while not self.stop_flag:
            try:
                ...
                if self.state == State.IDLE:
                    ...
                    # ━━━━━━ V32 JIT开仓+JIT平仓: 检测spread→紧贴市场挂单→2秒等待→撤单 ━━━━━━
...
def main():
    p = argparse.ArgumentParser(description="V32 阶梯建仓+逐层平仓")
    ...
    print(f"  JIT: spread≥{args.open_spread}bps(+{args.spread_buffer}缓冲) → 紧贴挂单2秒")
