#!/usr/bin/env python3
"""
Extended + Lighter 价差收敛套利 V17（Binance 领先定价 + 双向 Maker）

在 V16 基础上根本性升级 —— 引入 Binance 作为领先价格指标:
  - Binance 领先定价: 用 Binance futures bookTicker (免费/无API Key) 作为
    全球最快的 BTC 价格参考, 比 Lighter/Extended 快 200-1000ms
  - 逆选保护升级: 从 Lighter 漂移检测 → Binance 漂移检测, 反应速度提升 3-5x
  - 平仓恢复 maker 优先: Binance 价格稳定时安心等 maker 成交(省 2.5bps),
    Binance 出现波动时立即 taker 兜底
  - 波动率感知: 根据 Binance 30s 实时波动率动态调整挂单价差宽度
  - 继承 V16 全部 bug 修复(4单重复/单腿/撤单失败保护)
  - 保留 V14/V15 全部功能

策略（Binance 领先定价 + 持续做市）：
  定价参考: Lighter VWAP 计算挂单价, Binance 实时校验方向
  逆选保护: Binance bookTicker 回调, 偏移>1.0bps 立即撤单(比 V16 快 3-5x)
  挂单: 持续在 Extended 维护 buy + sell 双向 POST_ONLY maker 单
  成交: 任一侧成交后撤另一侧 → Lighter taker 对冲
  平仓: 分批触发 → maker 优先(Binance 稳定时等 10-15s) → taker 兜底
  波动率: Binance 高波动 → 加宽价差/缩短挂龄, 低波动 → 收窄/延长
...
class TakerBot:
    ...
    def __init__(
        ...
        self._maker_close_max_wait: float = 15.0
    ...
    def _get_bn_maker_close_timeout(self) -> float:
        """根据 Binance 波动率动态调整 maker 平仓等待时间"""
        bf = self.binance_feed
        if not bf or not bf.connected:
            return 5.0
        vol = bf.realized_vol_bps
        if vol >= self._bn_vol_wide_threshold:
            ...
            return self._maker_close_max_wait
        return 8.0
    ...
    async def _handle_dm_fill_inner(self) -> bool:
        ...
        if fill_price and l_vwap > 0:
            ...
            self.logger.info(
                ...
                f"深度={l_total_depth}")
            _catastrophic_threshold = -self._neg_spread_reverse_bps
            if est_spread < _catastrophic_threshold:
                self.logger.warning(
                    f"[双向Maker对冲] 逆选反转={est_spread:.1f}bps "
                    f"< 阈值{_catastrophic_threshold:.1f}bps → "
                    f"Extended紧急反向平仓(避免更大亏损)")
                ...
                try:
                    ...
                    if uw_fill and uw_fill > 0:
                        if fill_side == "buy":
                            ...
                            self.extended_position += uw_fill
                        self.logger.info(
                            f"[双向Maker对冲] Extended反向平仓成功 "
                            f"{unwind_side} {uw_fill} (严重逆选{est_spread:.1f}bps)")
                    else:
                        self.logger.error(
                            "[双向Maker对冲] Extended反向平仓失败 → 对账")
                        self._force_reconcile = True
                except Exception as e:
                    self.logger.error(
                        f"[双向Maker对冲] Extended反向平仓异常: {e}")
                    self._force_reconcile = True
                self._reset_dm_fill_state()
                return False
            if est_spread < 0:
                self.logger.info(
                    f"[双向Maker对冲] 价差为负={est_spread:.1f}bps "
                    f"但未达灾难阈值, 继续Lighter对冲")
    ...
    async def _main_loop_wrapper(self):
        mode_desc = "★ V17-Binance领先做市" if self.maker_mode else "双Taker"
        ...
        self.logger.info(
            f"启动 Extended+Lighter V17 {mode_desc}  {self.symbol}  "
        ...
        if self.maker_mode:
            self.logger.info(
                f"  V17-Binance做市: 窗口={self._dyn_window}s P{self._dyn_percentile:.0f} "
            ...
            try:
                ...
                if self.state == State.IDLE:
                    ...
                    if self.maker_mode and not self.dry_run:
                        ...
                        pricing_spread = max(
                            max(dyn_threshold, fee_floor) + _bn_adj,
                            fee_floor)
...
def main():
    ...
    if args.no_maker:
        ...
        bot._dyn_fee_floor = 2.0
        print(f"[模式] ★ V17-Binance领先做市+Taker即时平仓 (往返≈{bot.ROUND_TRIP_FEE_BPS:.1f}bps)")
        ...
        print(f"  费用底线: {bot._dyn_fee_floor:.1f}bps")
