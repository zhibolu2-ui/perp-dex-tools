...
class PendingMaker:
    ...
    phase: str = "open"
    l_ref_at_place: Optional[Decimal] = None
...
class ExtMakerBot:
    ...
    def __init__(
        ...
        self.csv_path = f"logs/arb_v8_{self.symbol}_trades.csv"
        self.log_path = f"logs/arb_v8_{self.symbol}.log"
        self._init_csv()
    ...
    def _init_logger(self) -> logging.Logger:
        lg = logging.getLogger(f"arb_v8_{self.symbol}")
    ...
    def _on_extended_order_event(self, event: dict):
        ...
        if self._open_buy and order_id == self._open_buy.order_id:
            self._handle_open_fill(event, "buy")
            return
        if self._open_sell and order_id == self._open_sell.order_id:
            self._handle_open_fill(event, "sell")
            return
    ...
    def _handle_open_fill(self, event: dict, side: str):
        """Handle fill events for dual-side open makers."""
        try:
            ...
            if status == "OPEN":
                self.logger.info(
                    f"[Extended] Open {side} maker 已上线 id={maker.order_id}")
                return
            ...
            if status in ("FILLED", "PARTIALLY_FILLED"):
                ...
                if avg_price_str:
                    try:
                        maker.avg_price = Decimal(str(avg_price_str))
                    except Exception:
                        pass
                ...
                if maker.hedge_qty_needed >= self.order_size * Decimal("0.01"):
                    ...
                    if other:
                        ...
                        asyncio.ensure_future(
                            self._cancel_extended_order(other.order_id))

                    self._open_buy = None
                    self._open_sell = None
                    self.state = State.HEDGING
                    self.logger.info(
                        f"[Extended FILL] {side} filled={filled_size} "
                        f"avg={avg_price_str} 待对冲={maker.hedge_qty_needed}")
                    self._fill_event.set()
                    asyncio.ensure_future(self._fast_hedge())

            elif status == "CANCELED":
                if filled_size > 0 and filled_size > maker.filled_qty:
                    self._handle_open_fill({**event, "status": "FILLED"}, side)
                    return
                if time.time() < self._cancel_grace_until:
                    return
                self.logger.info(
                    f"[Extended] Open {side} maker 已取消 (filled={filled_size})")
    ...
    async def _fast_hedge(self):
        async with self._hedge_lock:
            ...
            if phase == "open":
                ...
                net_bps = self._log_ref_pnl(x_fill_price, l_actual, actual_fill)
    ...
    def _calc_open_targets(
        ...
        """Calculate BOTH buy and sell Extended maker targets for opening.
        Returns (buy_target, sell_target).
        Each is (side, direction, price) or None if not viable.
        POST_ONLY constraint: buy < x_ask, sell > x_bid."""
        spread_bps = Decimal(str(self.target_spread))
        min_profit = l_bid * Decimal("0.5") / Decimal("10000")

        buy_target_price = self._round_extended_price(
            l_bid * (Decimal("1") - spread_bps / Decimal("10000")))
        buy_price = min(buy_target_price, x_ask - self.extended_tick_size)
        buy_result = None
        if l_bid - buy_price >= min_profit and buy_price > 0 and buy_price < x_ask:
            buy_result = ("buy", "long_X_short_L", buy_price)

        sell_target_price = self._round_extended_price(
            l_ask * (Decimal("1") + spread_bps / Decimal("10000")))
        sell_price = max(sell_target_price, x_bid + self.extended_tick_size)
        sell_result = None
        if sell_price - l_ask >= min_profit and sell_price > x_bid and sell_price > 0:
            sell_result = ("sell", "long_L_short_X", sell_price)

        return buy_result, sell_result

    def _calc_decay_bonus(self, decay_budget: Decimal) -> Tuple[Decimal, str]:
        """Based on hold duration, progressively relax max_gap.

        decay_budget = fee_cost + profit_target, representing the total
        margin that can be gradually sacrificed to avoid a timeout flatten.
        P4 (worst case) gives up the entire budget → accept breakeven.

        When both fees and profit are 0 (V8 default), a minimum decay
        of 0.5 bps of the reference price is used to ensure the mechanism
        still provides progressively easier exits over time.
        """
        if decay_budget <= 0:
            if self._open_x_refs:
                ref = sum(p * q for p, q in self._open_x_refs) / sum(
                    q for _, q in self._open_x_refs)
            else:
                ref = Decimal("70000")
            decay_budget = ref * Decimal("0.5") / Decimal("10000")

        hold_sec = time.time() - self.position_opened_at if self.position_opened_at else 0
        if hold_sec < 300:
            return Decimal("0"), "P1"
        elif hold_sec < 600:
            return decay_budget * Decimal("0.3"), "P2"
        elif hold_sec < 900:
            return decay_budget * Decimal("0.7"), "P3"
        else:
    ...
    async def _update_maker(
        ...
        l_ref_now: Optional[Decimal] = None,
    ) -> bool:
        now = time.time()
        ...
        if self._pending is not None:
            ...
            urgent = not same_side or price_diff >= self.extended_tick_size * 5
    ...
    async def _update_open_side(
        ...
        direction: str, l_ref: Decimal,
    ) -> bool:
        """Place or reprice one side of the dual open makers."""
        now = time.time()
        ...
        if maker is not None:
            ...
            if price_diff < self.extended_tick_size:
                return True

            urgent = price_diff >= self.extended_tick_size * 5
            if not urgent and now - last_reprice < self.reprice_interval:
                return True

            self._cancel_grace_until = time.time() + 2.0
            saved_id = maker.order_id
            cancel_ok = await self._cancel_extended_order(maker.order_id)
            if not cancel_ok:
                return True
            await asyncio.sleep(0.15)
            ...
            if current is None or current.order_id != saved_id:
                return True
        ...
        oid = await self._place_extended_maker(side, self.order_size, target_price)
        if oid:
            if self.state != State.QUOTING:
                self.logger.warning(
                    f"[竞态保护] 下单期间状态变为 {self.state.value}, "
                    f"撤销刚下的 {side} maker id={oid}")
                self._cancel_grace_until = time.time() + 2.0
                await self._cancel_extended_order(oid)
                return True

            new_maker = PendingMaker(
                order_id=oid, side=side, direction=direction,
                qty_target=self.order_size, price=target_price,
                placed_at=now, phase="open", l_ref_at_place=l_ref,
            )
            if side == "buy":
    ...
    async def _cancel_open_makers(self):
        """Cancel both open makers (used for cleanup).
        Only clears references for unfilled orders; filled ones are
        left for the fill handler to transfer to _pending."""
        for side, maker in [("buy", self._open_buy), ("sell", self._open_sell)]:
            if maker and maker.filled_qty == 0:
                self._cancel_grace_until = time.time() + 2.0
                await self._cancel_extended_order(maker.order_id)
                await asyncio.sleep(0.15)
            ...
            if fd is not None:
                ...
                if filled_base is not None:
                    ...
                    if ws_filled >= qty * Decimal("0.5") and ws_filled <= qty * Decimal("1.5"):
                        ...
                        return ws_filled
                    elif ws_filled > qty * Decimal("1.5"):
                        self.logger.warning(
                            f"[Lighter] WS fill 量异常: {ws_filled} vs 预期 {qty}, "
                            f"忽略(可能是旧订单), 回退 REST")
                        self._lighter_fill_data = None
        ...
        try:
            ...
            if result.success:
                ...
                if self._x_ioc_fill_qty > 0:
                    ...
                    self.logger.warning(
                        f"[Extended] IOC 已确认但 fill=0 (CANCELED/EXPIRED)")
                    self._x_ioc_pending_id = None
                    return Decimal("0")
                else:
                    self.logger.warning(
                        f"[Extended] IOC 超时未确认, 假定成交 {qty_r}")
                    actual = qty_r
                self._x_ioc_pending_id = None
                return actual
    ...
    async def _emergency_flatten(self):
        try:
            ...
            if abs(self.extended_position) > 0:
                ...
                flatten_qty = abs(self.extended_position)
                x_bid, x_ask = self._get_extended_ws_bbo()
                x_ref = x_bid if side == "sell" else x_ask
                if x_ref:
                    self.logger.info(
                        f"[紧急平仓] Extended IOC {side} {flatten_qty} @ {x_ref}")
                    ioc_fill = await self._place_extended_ioc(side, flatten_qty, x_ref)
                    if ioc_fill is not None and ioc_fill > 0:
                        if side == "sell":
                            ...
                            self.extended_position += ioc_fill
                        self.logger.info(
                            f"[紧急平仓] Extended 仓位更新: {self.extended_position}")
                    elif ioc_fill is not None and ioc_fill == 0:
                        self.logger.warning(
                            f"[紧急平仓] Extended IOC 未成交! 仍有 {flatten_qty} 需手动处理")
                else:
    ...
    async def _main_loop_wrapper(self):
        ...
        if not self.dry_run:
            ...
            if abs(self.lighter_position) > 0 or abs(self.extended_position) > 0:
                ...
                x_est = (x_bid_s + x_ask_s) / 2 if x_bid_s and x_ask_s else Decimal(str(ls.mid or 0))
                ...
                if x_est > 0 and l_est > 0:
                    ...
                    self.logger.warning(
                        "[启动] 行情不可用, 无法估算开仓参考, 平仓 maker 不会启动")
            ...
            try:
                ...
                if data_stale and not self.dry_run:
                    ...
                    if (self._pending or has_open) and not self._maker_pulled:
                        self.logger.warning("[数据过期] 撤回所有 Extended 挂单")
                        if has_open:
                            await self._cancel_open_makers()
                        if self._pending:
                            self._cancel_grace_until = time.time() + 2.0
                ...
                if now - last_status_log > 15:
                    ...
                    if self._open_buy:
                        pend += f" B@{self._open_buy.price}"
                    if self._open_sell:
                        pend += f" S@{self._open_sell.price}"
                    basis_info = ""
                    if (self.state == State.IN_POSITION
                        ...
                        _decay_b, _dphase = self._calc_decay_bonus(_fee + _prof)
                        _max_gap += _decay_b
                ...
                if self.state == State.QUOTING:
                    ...
                    for side_name, maker, l_ref_now in [
                        ("buy", self._open_buy, l_bid),
                        ("sell", self._open_sell, l_ask),
                    ]:
                        if (maker and maker.l_ref_at_place is not None
                                and maker.filled_qty == 0):
                            l_move = abs(l_ref_now - maker.l_ref_at_place)
                            l_move_bps = float(
                                l_move / maker.l_ref_at_place * 10000)
                            if l_move_bps > as_threshold:
                                self.logger.warning(
                                    f"[逆选保护] {side_name} Lighter移动"
                                    f" {l_move_bps:.1f}bps, 撤单")
                                self._cancel_grace_until = time.time() + 2.0
                    ...
                    if not self.dry_run:
                        ...
                        if now - last_status_log > 10:
                            parts = []
                            if buy_target:
                                parts.append(f"B@{buy_target[2]}")
                            if sell_target:
                                parts.append(f"S@{sell_target[2]}")
                            self.logger.info(
                        ...
                        if actual_fill is not None:
                            ...
                            if remaining >= self.order_size * Decimal("0.01"):
                                ...
                                if self.position_direction is None:
                                    self.position_direction = direction
                                self.logger.warning(
                                    f"[HEDGE FAIL] 仍有部分仓位 X={self.extended_position} "
                                    f"L={self.lighter_position}, 转 IN_POSITION 管理")
                                self.state = State.IN_POSITION
                            else:
                                self._reset_position_state()
                                self.state = State.QUOTING
                ...
                # ━━━━━━ STATE: CLOSING_HEDGE ━━━━━━
                    ...
                    async with self._hedge_lock:
                        ...
                        if actual_fill is not None:
                            ...
                            net_bps = self._log_ref_pnl(x_fill_price, l_actual_close, actual_fill)
                            self.cumulative_net_bps += net_bps
                            self.logger.info(
...
def main():
    ...
    p.add_argument("--target-spread", type=float, default=1.5,
                   help="开仓最低利润目标 (bps, 0费用所以低门槛即可)")
    ...
    p.add_argument("--interval", type=float, default=0.1,
                   help="主循环间隔 (秒)")
    p.add_argument("--reprice-interval", type=float, default=3.0,
                   help="最小调价间隔 (秒, Extended cancel+replace 较慢)")
    p.add_argument("--ladder-step", type=float, default=0.0,
                   help="阶梯加仓每层间距 (bps, 0=关闭, 如 2 表示每扩大 2bps 加一层)")
