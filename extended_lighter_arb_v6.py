#!/usr/bin/env python3
"""
Extended + Lighter 价差收敛套利 V6（Extended Maker + Lighter Taker — 零费用）

策略：
  持续在 Extended 维护 POST_ONLY maker 挂单，价格基于 Lighter 深度 BBO 计算。
  当挂单被成交 → 立刻在 Lighter IOC taker 对冲。
  持仓后在 Extended 维护平仓 maker 挂单，被成交后 Lighter 对冲平仓。

费用: ~0 bps 往返 (Extended maker 0% + Lighter taker 0%)

与 V5 的区别：
  V5: Lighter maker (0.4bps) + Extended taker (2.5bps) = ~5.8bps 往返
  V6: Extended maker (0%)    + Lighter taker (0%)    = ~0bps  往返

用法:
  python extended_lighter_arb_v6.py --symbol BTC --size 0.002 --target-spread 1.5
  python extended_lighter_arb_v6.py --symbol ETH --dry-run
"""
...
class ExtMakerBot:
    ...
    def __init__(
        ...
        self.csv_path = f"logs/arb_v6_{self.symbol}_trades.csv"
        self.log_path = f"logs/arb_v6_{self.symbol}.log"
        self._init_csv()
    ...
    def _init_logger(self) -> logging.Logger:
        lg = logging.getLogger(f"arb_v6_{self.symbol}")
    ...
    async def _fast_hedge(self):
        async with self._hedge_lock:
            ...
            if phase == "open":
                ...
                net_bps = self._log_ref_pnl(x_fill_price, l_ref)
    ...
    def _calc_maker_target(
        ...
        """Calculate Extended maker order: side, direction, price.
        POST_ONLY: buy < x_ask, sell > x_bid."""
        spread_bps = Decimal(str(self.target_spread))

        buy_target = self._round_extended_price(
            l_bid * (Decimal("1") - spread_bps / Decimal("10000")))
        sell_target = self._round_extended_price(
            l_ask * (Decimal("1") + spread_bps / Decimal("10000")))

        buy_price = min(buy_target, x_ask - self.extended_tick_size)
        sell_price = max(sell_target, x_bid + self.extended_tick_size)

        buy_profit = l_bid - buy_price
        sell_profit = sell_price - l_ask

        min_profit = l_bid * Decimal("0.5") / Decimal("10000")
        if buy_profit < min_profit and sell_profit < min_profit:
            return None

        if buy_profit >= sell_profit and buy_price > 0 and buy_price < x_ask:
            return ("buy", "long_X_short_L", buy_price)
        elif sell_price > x_bid and sell_price > 0:
            return ("sell", "long_L_short_X", sell_price)
        return None
    ...
    def _calc_close_maker_target(
        ...
        """Calculate close maker price with profitability constraint."""
        profit_buffer = Decimal(str(self.close_profit_bps)) / Decimal("10000")

        if self.extended_position > 0:
            close_side = "sell"
            min_sell = x_bid + self.extended_tick_size
            if self._open_x_refs and self._open_l_refs:
                ...
                min_sell = x_avg + l_ask - l_avg + x_avg * profit_buffer
            price = max(x_bid + self.extended_tick_size, min_sell)
            if x_ask - x_bid > self.extended_tick_size:
                price = max(price, x_ask - self.extended_tick_size)
            return (close_side, self._round_extended_price(price))

        elif self.extended_position < 0:
            close_side = "buy"
            max_buy = x_ask - self.extended_tick_size
            if self._open_x_refs and self._open_l_refs:
                x_avg = sum(p * q for p, q in self._open_x_refs) / sum(q for _, q in self._open_x_refs)
                l_avg = sum(p * q for p, q in self._open_l_refs) / sum(q for _, q in self._open_l_refs)
                max_buy = x_avg + l_bid - l_avg - x_avg * profit_buffer
            price = min(x_ask - self.extended_tick_size, max_buy)
            if price <= 0:
                return None
            return (close_side, self._round_extended_price(price))
    ...
    async def _main_loop_wrapper(self):
        self.logger.info(
            f"启动 V6 Extended-Maker 套利  {self.symbol}  "
        ...
        while not self.stop_flag:
            try:
                ...
                if self.state == State.QUOTING:
                    ...
                    target = self._calc_maker_target(l_bid, l_ask, x_ws_bid, x_ws_ask)
                    if target and not self.dry_run:
                        ...
                        if now - last_status_log > 10:
                            self.logger.info(
                                f"[DRY-RUN] {t_side}@{t_price} dir={t_dir}")
                    elif not target and self._pending:
                        self.logger.info("[QUOTING] 无合适价差, 撤回挂单")
                        self._cancel_grace_until = time.time() + 2.0
                        await self._cancel_extended_order(self._pending.order_id)
                        await asyncio.sleep(0.2)
                        if self._pending and self._pending.filled_qty <= self._pending.hedged_qty:
                ...
                # ━━━━━━ STATE: IN_POSITION ━━━━━━
                    ...
                    if x_zero != l_zero and not self.dry_run:
                        self.logger.warning(
                            f"[单边仓位] X={self.extended_position} L={self.lighter_position}")
                        await self._emergency_flatten()
                        self._pending = None
                        self._reset_position_state()
                        self.state = State.QUOTING
...
def main():
    p = argparse.ArgumentParser(
        description="V6 Extended Maker + Lighter Taker (零费用套利)")
    ...
    p.add_argument("--target-spread", type=float, default=1.5,
                   help="开仓最低利润目标 (bps)")
    p.add_argument("--close-profit-bps", type=float, default=0.5,
                   help="平仓最低利润缓冲 (bps)")
    ...
    bot = ExtMakerBot(
        ...
        close_profit_bps=max(args.close_profit_bps, 0.1),
