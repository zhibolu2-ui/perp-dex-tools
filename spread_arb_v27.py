#!/usr/bin/env python3
"""
V27: 双腿Taker套利 + 阶梯建仓 + 快速平仓

核心: 检测到价差 → 同时在Lighter和Extended发IOC taker → 即时锁定价差
  - 不挂maker单, 不等待被吃, 主动出手
  - 开仓: Lighter IOC sell(吃bid) + Extended IOC buy(吃ask) 同时发出
  - 平仓: Lighter IOC buy + Extended IOC sell (快速3层+深度观察)

费用:
  Lighter 免费账户 taker: 0 bps  |  Extended taker: 2.25 bps

用法:
  python spread_arb_v27.py --symbol BTC --size 0.002 --max-position 0.01 --open-spread 4.5 --close-spread 0 --ladder-step 1.5
"""
...
class SpreadArbBot:
    ...
    def __init__(
        ...
        # ── V27: Ladder level (开仓+1, 逐层平仓-1, 全平归零) ──
        self._ladder_level: int = 0

        # ── Logging / CSV ──
        os.makedirs("logs", exist_ok=True)
        self.csv_path = f"logs/spread_v27_{self.symbol}_trades.csv"
        self.log_path = f"logs/spread_v27_{self.symbol}.log"
        self._init_csv()
    ...
    def _init_logger(self) -> logging.Logger:
        lg = logging.getLogger(f"v27_{self.symbol}")
    ...
    def _on_lighter_fill(self, od: dict):
        """V27: 只处理平仓IOC的WS确认 + 非预期成交。开仓用双腿taker不经过WS。"""
        filled_base = Decimal(od.get("filled_base_amount", "0"))
        ...
        if self._in_flatten:
            ...
            if _is_late_flatten:
                return
            self.logger.info(
                f"[Lighter] WS成交通知(双腿taker已处理, 忽略): {side} {filled_base} @ {avg_price}")
    ...
    async def _lighter_ioc_open(self, side: str, qty: Decimal, price: Decimal):
        """Lighter IOC for opening. Returns (success, error_msg)."""
        is_ask = side == "sell"
        coi = int(time.time() * 1000) * 10 + (1 if is_ask else 2)
        await self._lighter_rate_wait()
        try:
    ...
    async def _dual_taker_open(self, l_side: str, l_ref: Decimal,
                                x_side: str, x_ref: Decimal):
        """V27: 同时发出Lighter IOC + Extended IOC, 并行执行。
        Returns: dict(成功) / "SKIP"(双失败) / "REPAIR"(单腿失败)"""
        slip = Decimal("0.002")
        if l_side == "sell":
        ...
        _l_task = asyncio.create_task(
            self._lighter_ioc_open(l_side, self.order_size, l_price))
        _x_task = asyncio.create_task(
            self._hedge_extended(x_side, self.order_size, x_ref))

        _l_result, _x_fill = await asyncio.gather(_l_task, _x_task, return_exceptions=True)
        ...
        if isinstance(_x_fill, Exception):
            _x_fill = None

        # ── 双腿都成功 ──
        if _l_ok and _x_fill is not None:
            if l_side == "sell":
            ...
            if not self._x_ioc_confirmed.is_set():
                try:
                    await asyncio.wait_for(self._x_ioc_confirmed.wait(), timeout=0.5)
                except asyncio.TimeoutError:
                    pass
            if self._x_ioc_fill_price and self._x_ioc_fill_price > 0:
                _x_fill = self._x_ioc_fill_price

            return {"l_side": l_side, "l_price": l_ref, "x_side": x_side,
                    "x_price": _x_fill, "ms": _ms}

        # ── Lighter API接受但Extended失败 → 对账修复(Lighter可能没实际成交) ──
        if _l_ok and _x_fill is None:
            self.logger.error(
                f"[开仓] Lighter API接受但Extended失败! → REPAIR对账  (L_{l_side}@{l_ref})")
            return "REPAIR"

        # ── Extended成功但Lighter失败 → 对账修复(Extended已更新仓位) ──
        if not _l_ok and _x_fill is not None:
            self.logger.error(
                f"[开仓] Extended成功但Lighter失败({_l_err})! → REPAIR对账  (X_{x_side}@{_x_fill})")
            return "REPAIR"
    ...
    async def _close_position(self, l_bid: Decimal, l_ask: Decimal,
        ...
        self._csv_row([
            ...
            f"v27_close_{self._trade_seq:04d}",
            f"close_{l_side}",
    ...
    async def _v23_record_pre_trade_balance(self):
        ...
        try:
            ...
            if _l is not None and _x is not None:
                self.logger.info(
                    f"[V27] 开仓后余额快照: L={_l} X={_x} 合计={_l+_x}")
        except Exception as e:
            self.logger.warning(f"[V27] 开仓余额快照失败: {e}")
    ...
    async def _v23_verify_close_pnl(self, formula_net_bps: float, formula_dollar: float,
        ...
        try:
            ...
            if _pre_l is None or _pre_x is None:
                self.logger.info(
                    f"[V27] 平仓后余额: L={_l_after} X={_x_after} (无开仓基准, 跳过对比)")
            ...
            self.logger.warning(f"[V27] 余额校验异常: {e}")
            return "", "", "", ""
    ...
    async def _main_loop(self):
        ...
        self.logger.info(
            f"启动 V27 双腿Taker套利  {self.symbol}  "
        ...
        if not self.dry_run:
            ...
            if _l_bal is not None and _x_bal is not None:
                self.logger.info(
                    f"[V27] 初始余额: Lighter={_l_bal}  Extended={_x_bal}  "
                    f"合计={_l_bal + _x_bal}")
            else:
                self.logger.warning(
                    f"[V27] 初始余额部分获取失败: L={_l_bal} X={_x_bal}")
        ...
        while not self.stop_flag:
            try:
                ...
                if self.state == State.IDLE:
                    ...
                    # ── V27 快速平仓+深度观察: 先快速平60%, 等几秒看是否继续收敛 ──
                    if _has_pos:
                        if self.lighter_position < 0:
                    ...
                    # ── V27 双腿Taker开仓: 检测可执行spread → 同时IOC ──
                    ...
                    _sell_spread = float((l_bid - ext_ask) / ext_ask * 10000) if ext_ask > 0 else 0
                    _buy_spread = float((ext_bid - l_ask) / l_ask * 10000) if l_ask > 0 else 0
                    ...
                    if _open_side:
                        _x_side = "buy" if _open_side == "sell" else "sell"
                        self.logger.info(
                            f"[开仓] {_open_side}价差={_open_spread:.1f}bps ≥ {effective_bps:.1f}bps  "
                            f"L={_open_l_ref} X={_open_x_ref} → 并行双腿IOC!")

                        result = await self._dual_taker_open(
                            _open_side, _open_l_ref, _x_side, _open_x_ref)
                        ...
                        _l_price = result["l_price"]
                        _x_fill = result["x_price"]
                        _hedge_ms = result["ms"]
                        _l_s = result["l_side"]

                        if _l_s == "sell":
                            _spread = float((_l_price - _x_fill) / _x_fill * 10000)
                        else:
                            _spread = float((_x_fill - _l_price) / _l_price * 10000)
                        _fee = float(FEE_EXTENDED_TAKER + FEE_LIGHTER_MAKER) * 10000
                        _net = _spread - _fee
                        self.cumulative_net_bps += _net
                        _dollar = _net / 10000 * float(self.order_size) * float(_x_fill)
                        self.cumulative_dollar_pnl += _dollar
                        self._consecutive_hedge_fails = 0
                        _did_trade = True

                        self.logger.info(
                            f"[开仓成交] L_{_l_s}@{_l_price} → X_{_x_side}@{_x_fill}  "
                        ...
                        self._csv_row([
                            datetime.now(_TZ_CN).isoformat(),
                            f"v27_open_{self._trade_seq:04d}", _l_s,
                            str(_l_price), str(_x_fill), str(self.order_size),
...
def main():
    p = argparse.ArgumentParser(description="V27 阶梯建仓+逐层平仓")
    ...
    print(f"[V27] 双腿Taker套利")
    print(f"  {args.symbol} size={args.size} max_pos={args.max_position}")
    print(f"  开仓: spread≥{args.open_spread}bps(+{args.spread_buffer}缓冲) → Lighter IOC + Extended IOC 同时出手")
