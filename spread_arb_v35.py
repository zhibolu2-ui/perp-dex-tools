#!/usr/bin/env python3
"""
V35: 分桶延迟学习 — spread x vol bucketed latency + 多级回退

基于 V34, 核心升级:
  1. BucketedLatencyPool: latency tracker 从全局方向 2 桶升级为
     方向 x spread_bucket x vol_bucket 分桶, 带三级回退
  2. 影子学习喂分桶: _process_shadow_learning 同时写 base + bucketed trackers
  3. 动态阈值从分桶取值: _calc_dynamic_threshold 按当前 regime 取最精确的 p75
  4. CSV 新增 4 列观察标签: spread_bucket, vol_bucket, depth_bucket, session_bucket
  5. 保留 V34 全部功能: VWAP/动态阈值/持续性过滤/分级平仓/影子学习

费用:
  Lighter 免费账户 maker: 0 bps | Extended taker: 2.25 bps

用法:
  python spread_arb_v35.py --symbol BTC --size 0.005 --max-position 0.02 --open-spread 4.2 --close-spread 0.4 --ladder-step 0.3 --open-jit-wait 1.5 --open-jit-price at_best
"""
...
class SpreadArbBot:
    ...
    def _calc_dynamic_threshold(self, side: str, qty: Decimal,
        ...
        V35: Uses bucketed latency pool with three-level fallback based on
        current spread regime and volatility regime.
        ...
        # V35: bucketed latency lookup
        s_bkt = self._bucket_spread(current_spread_raw)
        v_bkt = self._bucket_vol(self._short_vol_bps)
        latency_cost, _ = self.latency_pool.get_latency(side, s_bkt, v_bkt)
                ...
                if self.state == State.IDLE:
                    ...
                    if _jit_side and _jit_price and _jit_price > 0 and _persistent:
                        ...
                        self.logger.info(
                            ...
                            f"≥ thresh={_jit_threshold:.1f}bps(lat={_sig_lat:.1f}[L{_sig_lat_lvl}]+"
...
def main():
    ...
    rt_fee = fee_per * 2
    print(f"[V35] 分桶延迟学习 (spread x vol bucketed latency + 多级回退)")
