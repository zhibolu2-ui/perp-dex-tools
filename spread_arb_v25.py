...
用法:
  python spread_arb_v25.py --symbol BTC --size 0.002 --max-position 0.01 --open-spread 6 --close-spread 0 --ladder-step 1.5
"""
...
class SpreadArbBot:
    ...
    def __init__(
        ...
        # ── V25: Ladder level (开仓+1, 逐层平仓-1, 全平归零) ──
        self._ladder_level: int = 0

        # ── Logging / CSV ──
        os.makedirs("logs", exist_ok=True)
        self.csv_path = f"logs/spread_v25_{self.symbol}_trades.csv"
        self.log_path = f"logs/spread_v25_{self.symbol}.log"
        self._init_csv()
    ...
    def _init_logger(self) -> logging.Logger:
        lg = logging.getLogger(f"v25_{self.symbol}")
    ...
    def compute_maker_prices(
        ...
        V25: 内部价差 = open_spread + ladder_level * ladder_step + spread_buffer
    ...
    async def _close_position(self, l_bid: Decimal, l_ask: Decimal,
        ...
        self._csv_row([
            ...
            f"v25_close_{self._trade_seq:04d}",
            f"close_{l_side}",
    ...
    async def _v23_record_pre_trade_balance(self):
        ...
        try:
            ...
            if _l is not None and _x is not None:
                self.logger.info(
                    f"[V25] 开仓后余额快照: L={_l} X={_x} 合计={_l+_x}")
        except Exception as e:
            self.logger.warning(f"[V25] 开仓余额快照失败: {e}")
    ...
    async def _v23_verify_close_pnl(self, formula_net_bps: float, formula_dollar: float,
        ...
        try:
            ...
            if _pre_l is None or _pre_x is None:
                self.logger.info(
                    f"[V25] 平仓后余额: L={_l_after} X={_x_after} (无开仓基准, 跳过对比)")
            ...
            self.logger.warning(f"[V25] 余额校验异常: {e}")
            return "", "", "", ""
    ...
    async def _main_loop(self):
        ...
        self.logger.info(
            f"启动 V25 阶梯建仓+逐层平仓  {self.symbol}  "
        ...
        if not self.dry_run:
            ...
            if _l_bal is not None and _x_bal is not None:
                self.logger.info(
                    f"[V25] 初始余额: Lighter={_l_bal}  Extended={_x_bal}  "
                    f"合计={_l_bal + _x_bal}")
            else:
                self.logger.warning(
                    f"[V25] 初始余额部分获取失败: L={_l_bal} X={_x_bal}")
        ...
        while not self.stop_flag:
            try:
                ...
                if self.state == State.IDLE:
                    ...
                    # ── V25 快速平仓+深度观察: 先快速平60%, 等几秒看是否继续收敛 ──
                    if _has_pos:
                        if self.lighter_position < 0:
                ...
                # ━━━━━━ STATE: QUOTING ━━━━━━
                    ...
                    # V25: 1tick触发, 1.5s间隔, 单向安全modify(sell只上, buy只下)
...
def main():
    p = argparse.ArgumentParser(description="V25 阶梯建仓+逐层平仓")
