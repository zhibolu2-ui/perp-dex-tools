#!/usr/bin/env python3
"""
V22: 价差锁定 + 即时对冲 (Spread Lock + Instant Hedge)
...
用法:
  python spread_arb_v22.py --symbol BTC --size 0.001 --open-spread 5 --close-spread 0
  python spread_arb_v22.py --symbol BTC --size 0.001 --open-spread 5 --spread-buffer 2.0
"""
...
class SpreadArbBot:
    ...
    def __init__(
        ...
        self.csv_path = f"logs/spread_v22_{self.symbol}_trades.csv"
        self.log_path = f"logs/spread_v22_{self.symbol}.log"
        self._init_csv()
    ...
    def _init_logger(self) -> logging.Logger:
        lg = logging.getLogger(f"v22_{self.symbol}")
    ...
    async def _close_position(self, l_bid: Decimal, l_ask: Decimal,
        ...
        if not self._flatten_fill_event.is_set():
            _ws_tasks.append(asyncio.wait_for(
                self._flatten_fill_event.wait(), timeout=0.15))
        if not self._x_ioc_confirmed.is_set():
            _ws_tasks.append(asyncio.wait_for(
                self._x_ioc_confirmed.wait(), timeout=0.15))
        if _ws_tasks:
        ...
        self._csv_row([
            ...
            f"v22_close_{self._trade_seq:04d}",
            f"close_{l_side}",
...
def main():
    p = argparse.ArgumentParser(description="V22 即时对冲+spread guard")
