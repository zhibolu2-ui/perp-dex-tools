#!/usr/bin/env python3
"""
Extended + Lighter 价差收敛套利 V16（双向 Maker + 动态阈值）

在 V15 基础上核心升级：
  - 双向挂单策略：在 Extended 上同时维护 buy + sell 两张 POST_ONLY maker 单，
    哪边成交就立即撤另一边，然后在 Lighter 上 taker 对冲。
    消除方向翻转问题，降低撤单频率，成交概率翻倍。
  - 百分位动态阈值：根据近 N 分钟观测到的价差分布自适应调整开仓阈值。
  - 费用: 开仓 Extended maker(0) + Lighter taker(0.25bps), 平仓直接 taker(2.75bps)
    往返≈3.0bps, 最低挂单价差≥4.0bps
  - 逆向选择保护: Lighter 价格回调监听, 价格偏移>1.5bps 立即撤单
  - 保留 V14/V15 全部功能：分批平仓、加权收敛跟踪、阶梯加仓、回补、尘仓清理
  - --no-maker 回退到 V14 双 taker 逻辑（往返≈5.0 bps）

策略（持续做市 Maker 模式 + 逆选保护）：
  挂单: 持续在 Extended 维护 buy + sell 双向 POST_ONLY maker 单,
        价格 = Lighter VWAP ± 最低利润边距(≥4.0bps), 实时跟踪调整
  逆选保护: Lighter WS 回调检测价格偏移>1.5bps → 立即撤单(无需等主循环)
  成交: 任一侧成交后撤另一侧 → Lighter taker 对冲
        小幅负价差仍走 Lighter(fee≈0.25bps), 仅严重逆向才 Extended 紧急平仓
  平仓: 分批触发 → 直接双 taker 平仓(确定性>速度>费用节省)
  阶梯/回补: 沿用 V14 逻辑

费用:
  Maker 模式: ≈0.5 bps 往返 (Extended maker 0%, Lighter taker 0%, 含缓冲)
  Taker 回退: ≈5.0 bps 往返 (Extended taker 2.5bps×2)

用法:
  python extended_lighter_arb_v16.py --symbol BTC --size 0.005 --target-spread 3.0 \\
      --close-tiers "1.5:50,0.5:100" --dyn-percentile 75
  python extended_lighter_arb_v16.py --symbol ETH --no-maker --target-spread 7.5
"""
...
class TakerBot:
    ...
    def __init__(
        ...
        self._x_maker_close_timeout: float = 30.0

        # ── Dual-sided maker state (V16) ──
        ...
        self._dm_lighter_drift_bps: float = 2.5
        ...
        self._dyn_fee_floor: float = self.ROUND_TRIP_FEE_BPS_MAKER + 1.0
        ...
        self.csv_path = f"logs/arb_v16_{self.symbol}_trades.csv"
        self.log_path = f"logs/arb_v16_{self.symbol}.log"
        self._init_csv()
    ...
    def _init_logger(self) -> logging.Logger:
        lg = logging.getLogger(f"arb_v16_{self.symbol}")
    ...
    def _on_extended_order_event(self, event: dict):
        ...
        # ── Dual-sided maker tracking (V16 IDLE) ──
    ...
    def _calc_dual_maker_prices(
        ...
        buy_target = l_sell_ref * (Decimal("1") - sp)
        ...
        sell_target = l_buy_ref * (Decimal("1") + sp)
    ...
    async def _place_dm_order(
        ...
        try:
            ...
            self.logger.info(
                ...
                f"id={order_id} L_mid={self._dm_lighter_mid_at_place:.1f}")
    ...
    async def _open_position(
        ...
        async with self._trade_lock:
            ...
            if vwap_spread < self.ROUND_TRIP_FEE_BPS:
                self.logger.info(
                    f"[开仓] VWAP价差={vwap_spread:.1f}bps < "
                    f"费用{self.ROUND_TRIP_FEE_BPS}bps (L_VWAP={l_vwap:.1f} "
    ...
    async def _handle_dm_fill_inner(self) -> bool:
        ...
        if fill_price and l_vwap > 0:
            ...
            self.logger.info(
                ...
                f"深度={l_total_depth}")
            if est_spread < -self._max_slippage_bps:
                self.logger.warning(
                    f"[双向Maker对冲] 价差严重为负={est_spread:.1f}bps "
                    f"超过滑点保护{self._max_slippage_bps}bps! "
                    f"Extended紧急反向平仓")
                ...
                try:
                    ...
                    if uw_fill and uw_fill > 0:
                        if fill_side == "buy":
                            ...
                            self.extended_position += uw_fill
                        self.logger.info(
                            f"[双向Maker对冲] Extended紧急平仓成功 "
                            f"{unwind_side} {uw_fill}")
                    else:
                        self.logger.error(
                            "[双向Maker对冲] Extended紧急平仓失败 → 对账")
                        self._force_reconcile = True
                except Exception as e:
                    self.logger.error(
                        f"[双向Maker对冲] Extended紧急平仓异常: {e}")
                    self._force_reconcile = True
                self._reset_dm_fill_state()
                return False
            elif est_spread < 0:
                self.logger.info(
                    f"[双向Maker对冲] 价差小幅为负={est_spread:.1f}bps "
                    f"仍在Lighter对冲(fee≈0.25bps 优于Extended IOC≈2.5bps)")
        ...
        if l_fill and l_fill > 0:
            ...
            self._log_open_result(
                ...
                spread_bps, total_ms, False, "dual-maker")
    ...
    async def _close_position(
        ...
        async with self._trade_lock:
            ...
            if x_ok and l_ok:
                ...
                l_retry = await self._place_lighter_taker(close_l_side, x_fill, retry_ref)
                if not (l_retry and l_retry > 0):
                    self._force_reconcile = True
                return False
    ...
    async def _close_position_mt(
        ...
        async with self._trade_lock:
            ...
            maker_price = self._calc_proactive_maker_price(
                close_x_side, l_bid, l_ask, x_bid, x_ask,
                spread_bps=0.3)
            ...
            x_fill_qty, x_fill_price = await self._wait_maker_fill(
                order_id, self._x_maker_close_timeout)
            total_ms = (time.time() - t0) * 1000
            ...
            if x_fill_qty > 0:
                ...
                if l_fill and l_fill > 0:
                    ...
                    return self._finalize_close(
                        ...
                        l_fill, x_fill_qty, total_ms, batch_pct, "maker")
                else:
                ...
                self.logger.warning(
                    f"[平仓MT] maker {self._x_maker_close_timeout}s超时, "
                    f"回退双taker")
                return False
    ...
    def _finalize_close(self, tier_label, hedge_qty, l_fill_price, x_fill_price,
        ...
        net_bps = self._log_ref_pnl(
            x_fill_price, l_fill_price, hedge_qty, x_estimated=x_estimated)
    ...
    async def _main_loop_wrapper(self):
        mode_desc = "★ 双向Maker-V16" if self.maker_mode else "双Taker"
        ...
        self.logger.info(
            f"启动 Extended+Lighter V16 {mode_desc}  {self.symbol}  "
        ...
        if self.maker_mode:
            self.logger.info(
                f"  V16双向Maker: 窗口={self._dyn_window}s P{self._dyn_percentile:.0f} "
        ...
        while not self.stop_flag:
            try:
                ...
                if (not self.dry_run and now - last_reconcile > 30
                    ...
                    if self._x_dm_fill_event.is_set():
                        pass
                    elif ((self._x_maker_buy_id or self._x_maker_sell_id)
                ...
                if self.state == State.IDLE:
                    ...
                    # ── Maker mode: dual-sided order management (V16) ──
                    if self.maker_mode and not self.dry_run:
                        ...
                        _dm_min_age = 5.0
                        pricing_spread = max(dyn_threshold, fee_floor)
                        ...
                        # ── Lighter drift force cancel (逆向选择保护) ──
                        if self._dm_force_cancel:
                            ...
                            if _drift > self._dm_lighter_drift_bps:
                                self.logger.warning(
                                    f"[逆选保护] Lighter漂移{_drift:.1f}bps "
                                    f">{self._dm_lighter_drift_bps}bps 紧急撤单")
                                await self._cancel_all_dm_orders()
                                if self._x_dm_fill_event.is_set():
                                    ok = await self._handle_dm_fill()
                                    if ok:
                        ...
                        if self._x_maker_buy_id is not None and buy_price is not None:
                            ...
                            need_replace_buy = (
                                ...
                                and (buy_drift > self._price_tolerance_bps
                                     or buy_age > self._maker_max_age))
                        ...
                        if self._x_maker_sell_id is not None and sell_price is not None:
                            ...
                            need_replace_sell = (
                                ...
                                and (sell_drift > self._price_tolerance_bps
                                     or sell_age > self._maker_max_age))
                        ...
                        if self._x_maker_buy_id is None and buy_price is not None:
                        ...
                        if self._x_maker_sell_id is None and sell_price is not None:
                            if self._check_tx_rate():
                                await self._place_dm_order(
...
def main():
    p = argparse.ArgumentParser(
        description="V16 双向Maker策略套利 (Lighter+Extended)")
    ...
    if args.no_maker:
        ...
        bot._dyn_fee_floor = bot.ROUND_TRIP_FEE_BPS_MAKER + 1.0
        print(f"[模式] ★ V16双向Maker (往返≈{bot.ROUND_TRIP_FEE_BPS:.1f}bps)")
