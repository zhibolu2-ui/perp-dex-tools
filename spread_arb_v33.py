#!/usr/bin/env python3
"""
V33: JIT 开仓/平仓分离参数 + 定价模式可配 + 阶梯建仓

基于 V32, 核心优化:
  1. 开仓/平仓 JIT 等待时间分离: 平仓更短(1s), 开仓适中(1.5s)
  2. 定价模式可选: at_best(队列头) / improve(改善1tick) / behind(V32原逻辑)
  3. 平仓默认 improve 模式 — 改善 best price, 大幅提升填充率
  4. 开仓默认 at_best — 加入 best 队列, 平衡填充率与逆向选择

费用:
  Lighter 免费账户 maker: 0 bps | Extended taker: 2.25 bps

用法:
  python spread_arb_v33.py --symbol BTC --size 0.002 --max-position 0.01 --open-spread 4 --close-spread 0 --ladder-step 1.5 --open-jit-wait 1.5 --close-jit-wait 1.0 --open-jit-price at_best --close-jit-price improve
"""
...
class SpreadArbBot:
    ...
    def __init__(
        ...
        self.csv_path = f"logs/spread_v33_{self.symbol}_trades.csv"
        self.log_path = f"logs/spread_v33_{self.symbol}.log"
        self._init_csv()
    ...
    def _init_logger(self) -> logging.Logger:
        lg = logging.getLogger(f"v33_{self.symbol}")
    ...
    def compute_maker_prices(
        ...
        V32: 内部价差 = open_spread + ladder_level * ladder_step + spread_buffer
    ...
    async def _close_position(self, l_bid: Decimal, l_ask: Decimal,
        ...
        """V33逐层平仓: 直接taker IOC, 跳过不必要的cancel节省200ms。"""
        ...
        self._csv_row([
            ...
            f"v33_close_{self._trade_seq:04d}",
            f"close_{l_side}",
    ...
    async def _close_position_batch(
        ...
        self._csv_row([
            datetime.now(_TZ_CN).isoformat(),
            f"v33_bclose_{self._trade_seq:04d}",
            f"close_{l_side}",
            str(_l_actual), str(_x_actual), str(_actual_qty),
            f"{_spread_bps:.2f}", f"{_fee:.2f}", f"{_net:.2f}",
            f"{_ms:.0f}", f"{self.cumulative_net_bps:.2f}",
    ...
    async def _v23_record_pre_trade_balance(self):
        ...
        try:
            ...
            if _l is not None and _x is not None:
                self.logger.info(
                    f"[V33] 开仓后余额快照: L={_l} X={_x} 合计={_l+_x}")
        except Exception as e:
            self.logger.warning(f"[V33] 开仓余额快照失败: {e}")
    ...
    async def _v23_verify_close_pnl(self, formula_net_bps: float, formula_dollar: float,
        ...
        try:
            ...
            if _pre_l is None or _pre_x is None:
                self.logger.info(
                    f"[V33] 平仓后余额: L={_l_after} X={_x_after} (无开仓基准, 跳过对比)")
            ...
            self.logger.warning(f"[V33] 余额校验异常: {e}")
            return "", "", "", ""
    ...
    async def _main_loop(self):
        ...
        if not self.dry_run:
            ...
            if _l_bal is not None and _x_bal is not None:
                self.logger.info(
                    f"[V33] 初始余额: Lighter={_l_bal}  Extended={_x_bal}  "
                    f"合计={_l_bal + _x_bal}")
            else:
                self.logger.warning(
                    f"[V33] 初始余额部分获取失败: L={_l_bal} X={_x_bal}")
        ...
        while not self.stop_flag:
            try:
                ...
                if self.state == State.IDLE:
                    ...
                    # ── V32 快速平仓+深度观察: 先快速平60%, 等几秒看是否继续收敛 ──
                    if _has_pos:
                        if self.lighter_position < 0:
                            ...
                            if _still_has and self.close_wait_sec > 0:
                                ...
                                if _deep_ok:
                                    ...
                                    self.logger.info(
                                        ...
                                        f"≤ {self.close_deep_bps}bps → 批量平剩余 {_remain_qty}")
                    ...
                    # ━━━━━━ V33 JIT开仓: 检测spread→按模式定价→等待→撤单 ━━━━━━
                    ...
                    if _allow_sell and _sell_spread >= effective_bps:
                        ...
                        _jit_price = self._compute_jit_price(
                            "sell", l_bid, l_ask, self.open_jit_price_mode)
                        _jit_spread = _sell_spread
                    elif _allow_buy and _buy_spread >= effective_bps:
                        _jit_side = "buy"
                        _jit_price = self._compute_jit_price(
                            "buy", l_bid, l_ask, self.open_jit_price_mode)
                        _jit_spread = _buy_spread

                    if _jit_side and _jit_price and _jit_price > 0:
                        self.logger.info(
                            f"[JIT] {_jit_side} spread={_jit_spread:.1f}bps ≥ {effective_bps:.1f}bps  "
...
def main():
    p = argparse.ArgumentParser(description="V33 JIT分离参数 + 阶梯建仓")
    ...
    p.add_argument("--spread-buffer", type=float, default=0.0,
                   help="内部价差缓冲bps: 补偿延迟滑点 (默认0, 内部挂单按open-spread+buffer)")
    ...
    print(f"  开仓JIT: spread≥{args.open_spread}bps(+{args.spread_buffer}缓冲) → {args.open_jit_price}挂单{args.open_jit_wait}秒")
    print(f"  平仓: 批量taker IOC(多层合并1单, ~330ms)")
    print(f"  平仓: basis≤{args.close_spread} → 批量平≤3层(1单) → 等{args.close_wait}s → basis≤{args.close_deep}再批量平剩余")
    print(step_info)
