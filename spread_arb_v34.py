#!/usr/bin/env python3
"""
V34: 可执行净边际套利 — 从静态价差升级为动态阈值+分级对冲

基于 V33, 核心升级:
  1. Lighter VWAP: 利用完整订单簿深度计算 size-aware 可执行价差
  2. 延迟惩罚学习: 运行时统计 300ms 延迟导致的价差恶化 p75 分位数
  3. 动态入场阈值: 费用+延迟+滑点+波动+库存 五层成本叠加
  4. 持续性过滤: 要求 spread 连续 N 次采样满足，过滤瞬时假信号
  5. 短时波动追踪: EWMA 波动率用于动态调整阈值
  6. 分级平仓: 4 级对冲状态机 (Passive/Aggressive/Fast/Emergency)
  7. 增强日志: 每笔记录 observed/executable/predicted/realized 全链路数据

费用:
  Lighter 免费账户 maker: 0 bps | Extended taker: 2.25 bps

用法:
  python spread_arb_v34.py --symbol BTC --size 0.005 --max-position 0.02 --open-spread 4.2 --close-spread 0.4 --ladder-step 0.3 --open-jit-wait 1.5 --open-jit-price at_best
"""
...
class SpreadArbBot:
    ...
    def __init__(
        ...
        # ── V34: Latency tracker per side (learns from live trades) ──
        self.latency_trackers: Dict[str, LatencyTracker] = {
            "buy": LatencyTracker(window=150, cold_default=latency_cold),
            "sell": LatencyTracker(window=150, cold_default=latency_cold),
        }
        ...
        self.csv_path = f"logs/spread_v34_{self.symbol}_trades.csv"
        self.log_path = f"logs/spread_v34_{self.symbol}.log"
        self._init_csv()
    ...
    def _init_logger(self) -> logging.Logger:
        lg = logging.getLogger(f"v34_{self.symbol}")
    ...
    def _calc_dynamic_threshold(self, side: str, qty: Decimal) -> float:
        """Compute the minimum executable spread needed for a positive-EV trade.
        ...
        Static floor  = open_spread + ladder + spread_buffer  (user's subjective minimum)
        Dynamic cost  = close_target + fee_rt + latency + lighter_slip + extended_slip
                        + vol_penalty + inventory_penalty
        """
        ...
        fee_per_leg = float(FEE_EXTENDED_TAKER) * 10000  # 2.25 bps
        base_cost = fee_per_leg * 2                        # 4.5 bps round-trip

        latency_cost = self.latency_trackers[side].p75
        ...
        inv_cost = self.inv_weight * inv_ratio * 2.0  # max ~2bps at full inventory

        close_target = abs(self.close_spread_bps)

        dynamic_cost = (close_target + base_cost + latency_cost
    ...
    def _process_shadow_learning(self, now: float,
        ...
        while self._shadow_snapshots:
            ...
            tracker = self.latency_trackers[side]
            was_cold = not tracker.warm
            tracker.record(obs_bps, current_bps)

            if was_cold and tracker.just_warmed:
                self.logger.info(
                    f"[Shadow] {side} tracker已热启动: {tracker.summary()} "
                    f"(阈值将从冷启动值切换到学习值)")
    ...
    def _choose_hedge_mode(self, basis_bps: float,
        ...
        Uses the same cost components as _calc_dynamic_threshold so the close
        decision is calibrated to the same model that decided to open.
        """
        fee_rt = float(FEE_EXTENDED_TAKER) * 10000 * 2
        ...
        lat_cost = (self.latency_trackers["buy"].p75
                    + self.latency_trackers["sell"].p75) / 2
        vol_cost = self.vol_weight * self._short_vol_bps * 0.5

        residual = abs(basis_bps) - fee_rt - lighter_slip - ext_slip - lat_cost - vol_cost
    ...
    async def _close_position(self, l_bid: Decimal, l_ask: Decimal,
        ...
        self._csv_row([
            ...
            f"v34_close_{self._trade_seq:04d}",
            f"close_{l_side}",
    ...
    async def _close_position_batch(
        ...
        self._csv_row([
            datetime.now(_TZ_CN).isoformat(),
            f"v34_bclose_{self._trade_seq:04d}",
            f"close_{l_side}",
            str(_l_actual), str(_x_actual), str(_actual_qty),
            f"{_spread_bps:.2f}", f"{_fee:.2f}", f"{_net:.2f}",
            f"{_ms:.0f}", f"{self.cumulative_net_bps:.2f}",
    ...
    async def _v23_record_pre_trade_balance(self):
        ...
        try:
            ...
            if _l is not None and _x is not None:
                self.logger.info(
                    f"[V34] 开仓后余额快照: L={_l} X={_x} 合计={_l+_x}")
        except Exception as e:
            self.logger.warning(f"[V34] 开仓余额快照失败: {e}")
    ...
    async def _v23_verify_close_pnl(self, formula_net_bps: float, formula_dollar: float,
        ...
        try:
            ...
            if _pre_l is None or _pre_x is None:
                self.logger.info(
                    f"[V34] 平仓后余额: L={_l_after} X={_x_after} (无开仓基准, 跳过对比)")
            ...
            self.logger.warning(f"[V34] 余额校验异常: {e}")
            return "", "", "", ""
    ...
    async def _main_loop(self):
        ...
        if not self.dry_run:
            ...
            if _l_bal is not None and _x_bal is not None:
                self.logger.info(
                    f"[V34] 初始余额: Lighter={_l_bal}  Extended={_x_bal}  "
                    f"合计={_l_bal + _x_bal}")
            else:
                self.logger.warning(
                    f"[V34] 初始余额部分获取失败: L={_l_bal} X={_x_bal}")
            ...
            try:
                ...
                if now - last_status_log > 15:
                    self.logger.info(
                        ...
                        f"vol={self._short_vol_bps:.1f}bps  "
                        f"lat_b={self.latency_trackers['buy'].p75:.1f}/s={self.latency_trackers['sell'].p75:.1f}bps  "
                        f"累积=${self.cumulative_dollar_pnl:+.4f}")
                    last_status_log = now
                ...
                if self.state == State.IDLE:
                    ...
                    if _has_pos:
                        ...
                        if _should_close:
                            ...
                            _batch_qty = min(
                                _closable,
                                self.order_size * _fast_layers,
                            )
                            # V34: Choose hedge mode with actual qty
                            ...
                            self.logger.info(
                                ...
                                f"平{_fast_layers}/{_total_layers}层 qty={_batch_qty}  "
                                f"L={self.lighter_position} X={self.extended_position}  "
                            ...
                            if _still_has and self.close_wait_sec > 0:
                                ...
                                if _deep_ok:
                                    ...
                                    self.logger.info(
                                        ...
                                        f"mode={_deep_mode.value} 平剩余 {_remain_qty}")
                    ...
                    # V34: Dynamic threshold (replaces static effective_bps)
                    _sell_threshold = self._calc_dynamic_threshold("sell", self.order_size)
                    _buy_threshold = self._calc_dynamic_threshold("buy", self.order_size)
                    ...
                    if _jit_side and _jit_price and _jit_price > 0 and _persistent:
                        ...
                        self._signal_latency_penalty = self.latency_trackers[_jit_side].p75
                        ...
                        self.logger.info(
                            ...
                            f"≥ thresh={_jit_threshold:.1f}bps(lat={self.latency_trackers[_jit_side].p75:.1f}+"
                            f"vol={self._short_vol_bps:.1f})  "
                ...
                # ━━━━━━ STATE: HEDGING ━━━━━━
                    ...
                    if _hedge_ok and _x_fill_price:
                        ...
                        # V34: Feed realized spread to latency tracker (per-side)
                        if self._signal_observed_spread > 0 and _side in self.latency_trackers:
                            self.latency_trackers[_side].record(
                                self._signal_observed_spread, _spread)
                        ...
                        self._csv_row([
                            datetime.now(_TZ_CN).isoformat(),
                            f"v34_open_{self._trade_seq:04d}",
                            _side,
...
def main():
    p = argparse.ArgumentParser(description="V34 可执行净边际套利 (动态阈值+分级对冲)")
    ...
    # V34 new params
    p.add_argument("--vol-weight", type=float, default=1.0,
                   help="波动惩罚权重: vol_cost = weight * ewma_vol (默认1.0)")
    ...
    rt_fee = fee_per * 2
    print(f"[V34] 可执行净边际套利 (动态阈值+分级对冲)")
    print(f"  {args.symbol} size={args.size} max_pos={args.max_position}")
    print(f"  基础open_spread={args.open_spread}bps → 动态阈值=费用({rt_fee:.1f})+延迟(p75)+滑点(VWAP)+波动(EWMA)+库存")
    print(f"  持续性过滤: {args.persist_count}次/{args.persist_ms}ms")
    print(f"  平仓: basis≤{args.close_spread} → 分级(passive>{args.close_passive_edge}/agg>{args.close_aggressive_edge}/fast/emergency)")
    step_info = f"  阶梯间距={args.ladder_step}bps" if args.ladder_step > 0 else "  阶梯=关闭"
    print(step_info)
    print(f"  延迟冷启动={args.latency_cold}bps → 影子学习~6秒自动热启动  vol_w={args.vol_weight} inv_w={args.inv_weight}")
