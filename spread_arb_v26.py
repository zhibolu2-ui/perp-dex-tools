#!/usr/bin/env python3
"""
V26: 阶梯建仓 + 快速平仓+深度观察 + 即时对冲 + 余额校验

平仓策略:
  ≤3层: 价差收敛到close-spread时全部平仓
  4-5层: 快速平3层 → 等close-wait秒 → basis≤close-deep再平剩余, 否则保留

功能:
  - 阶梯建仓(逐层开仓, 每层间距ladder-step bps)
  - 余额校验P&L, CSV真实P&L列
  - WS即时对冲, IOC taker, spread-buffer, 实际成交价追踪
...
用法:
  python spread_arb_v26.py --symbol BTC --size 0.002 --max-position 0.01 --open-spread 6 --close-spread 0 --ladder-step 1.5
"""
...
class SpreadArbBot:
    ...
    def __init__(
        ...
        # ── V26: Ladder level (开仓+1, 逐层平仓-1, 全平归零) ──
        self._ladder_level: int = 0

        # ── V26: CLOSING state (maker平仓) ──
        self._l_close_client_id: Optional[int] = None
        self._l_close_order_idx: Optional[int] = None
        self._l_close_price = Decimal("0")
        self._l_close_placed_at: float = 0.0
        self._l_close_ext_bid_at_place = Decimal("0")
        self._l_close_ext_ask_at_place = Decimal("0")
        self._is_close_cycle: bool = False

        # ── Logging / CSV ──
        os.makedirs("logs", exist_ok=True)
        self.csv_path = f"logs/spread_v26_{self.symbol}_trades.csv"
        self.log_path = f"logs/spread_v26_{self.symbol}.log"
        self._init_csv()
    ...
    def _init_logger(self) -> logging.Logger:
        lg = logging.getLogger(f"v26_{self.symbol}")
    ...
    def compute_maker_prices(
        ...
        V26: 内部价差 = open_spread + ladder_level * ladder_step + spread_buffer
        随着仓位累积, 挂单距离逐层拉远, 只在价差更大时才继续开仓。
        """
    ...
    def compute_close_maker_price(
        self, ext_bid: Decimal, ext_ask: Decimal,
        l_bid: Decimal, l_ask: Decimal,
    ) -> Optional[Decimal]:
        """V26: maker平仓价格。short→挂buy略低于ext_bid, long→挂sell略高于ext_ask"""
        buffer = Decimal(str(self.spread_buffer_bps)) / Decimal("10000")
        if self.lighter_position < 0:
            price = self._round_lighter(ext_bid * (Decimal("1") - buffer))
            if price >= l_bid:
                price = l_bid - self.lighter_tick
            return price if price > 0 else None
        elif self.lighter_position > 0:
            price = self._round_lighter(ext_ask * (Decimal("1") + buffer))
            if price <= l_ask:
    ...
    def _on_lighter_fill(self, od: dict):
        ...
        is_our_close = (cid is not None and self._l_close_client_id is not None
                        and int(cid) == self._l_close_client_id)

        if (is_our_buy or is_our_sell or is_our_close) and not self._fill_event.is_set():
            if is_ask:
    ...
    def _on_lighter_open(self, od: dict):
        ...
        if cid is not None:
            ...
            elif self._l_close_client_id is not None and cid_int == self._l_close_client_id:
                self._l_close_order_idx = oidx
    ...
    async def _place_lighter_order(self, side: str, qty: Decimal, price: Decimal,
    ...
    async def _cancel_all_lighter(self):
        ...
        self._l_close_client_id = None
        self._l_close_order_idx = None
    ...
    async def _close_position(self, l_bid: Decimal, l_ask: Decimal,
        ...
        self._csv_row([
            ...
            f"v26_close_{self._trade_seq:04d}",
            f"close_{l_side}",
    ...
    async def _v23_record_pre_trade_balance(self):
        ...
        try:
            ...
            if _l is not None and _x is not None:
                self.logger.info(
                    f"[V26] 开仓后余额快照: L={_l} X={_x} 合计={_l+_x}")
        except Exception as e:
            self.logger.warning(f"[V26] 开仓余额快照失败: {e}")
    ...
    async def _v23_verify_close_pnl(self, formula_net_bps: float, formula_dollar: float,
        ...
        try:
            ...
            if _pre_l is None or _pre_x is None:
                self.logger.info(
                    f"[V26] 平仓后余额: L={_l_after} X={_x_after} (无开仓基准, 跳过对比)")
            ...
            self.logger.warning(f"[V26] 余额校验异常: {e}")
            return "", "", "", ""
    ...
    async def _main_loop(self):
        ...
        self.logger.info(
            f"启动 V26 阶梯建仓+逐层平仓  {self.symbol}  "
        ...
        if not self.dry_run:
            ...
            if _l_bal is not None and _x_bal is not None:
                self.logger.info(
                    f"[V26] 初始余额: Lighter={_l_bal}  Extended={_x_bal}  "
                    f"合计={_l_bal + _x_bal}")
            else:
                self.logger.warning(
                    f"[V26] 初始余额部分获取失败: L={_l_bal} X={_x_bal}")
        ...
        while not self.stop_flag:
            try:
                    ...
                    if _has_pos:
                        ...
                        if _should_close:
                            if (self._l_buy_client_id or self._l_sell_client_id
                                ...
                                await self._cancel_all_lighter()
                            _close_price = self.compute_close_maker_price(
                                ext_bid, ext_ask, l_bid, l_ask)
                            if _close_price:
                                _close_side = "buy" if self.lighter_position < 0 else "sell"
                                self._fill_event.clear()
                                self._fill_side = None
                                self._is_close_cycle = True
                                cid = await self._place_lighter_order(
                                    _close_side, self.order_size, _close_price,
                                    reduce_only=True)
                                if cid:
                                    self._l_close_client_id = cid
                                    self._l_close_price = _close_price
                                    self._l_close_placed_at = now
                                    self._l_close_ext_bid_at_place = ext_bid
                                    self._l_close_ext_ask_at_place = ext_ask
                                    self._last_modify_ts = now
                                    self.state = State.CLOSING
                                    self.logger.info(
                                        f"[IDLE→CLOSING] 价差收敛! basis={basis_bps:.1f}bps  "
                                        f"挂maker {_close_side} {self.order_size}@{_close_price}  "
                                        f"L={self.lighter_position} X={self.extended_position}  "
                                        f"ladder=L{self._ladder_level}")
                                else:
                                    self._is_close_cycle = False
                            continue
                ...
                # ━━━━━━ STATE: QUOTING ━━━━━━
                    ...
                    if need_requote and age >= 0.5 and _since_last_mod >= 0.5:
                    ...
                    try:
                        await asyncio.wait_for(self._fill_event.wait(), timeout=0.2)
                        if self._fill_event.is_set():
                            continue
                    except asyncio.TimeoutError:
                        pass

                # ━━━━━━ STATE: CLOSING (V26 maker平仓) ━━━━━━
                elif self.state == State.CLOSING:
                    if self._fill_event.is_set():
                        _f_side = self._fill_side
                        _snap_bid = self._fill_ext_bid
                        _snap_ask = self._fill_ext_ask
                        if _f_side == "buy" and _snap_bid and _snap_bid > 0:
                            _real_spread = float((_snap_bid - self._fill_price) / self._fill_price * 10000)
                        elif _f_side == "sell" and _snap_ask and _snap_ask > 0:
                            _real_spread = float((self._fill_price - _snap_ask) / _snap_ask * 10000)
                        else:
                            _real_spread = 0
                        self.logger.info(
                            f"[CLOSING] maker平仓成交! spread={_real_spread:.1f}bps → 即时对冲")
                        self.state = State.HEDGING
                        continue

                    # 价差回弹 → 取消平仓单, 保留仓位
                    if self.lighter_position < 0:
                        _spread_rebounded = basis_bps > self.open_spread_bps
                    else:
                        _spread_rebounded = basis_bps < -self.open_spread_bps
                    if _spread_rebounded:
                        self.logger.info(
                            f"[CLOSING] 价差回弹! basis={basis_bps:.1f}bps → 取消平仓单")
                        await self._cancel_all_lighter()
                        await asyncio.sleep(0.05)
                        if self._fill_event.is_set():
                            self.state = State.HEDGING
                    ...
                    # 超时
                    _close_age = now - self._l_close_placed_at
                    if _close_age > self.max_order_age:
                        self.logger.info(f"[CLOSING] 超时({_close_age:.0f}s), 撤单回IDLE")
                        await self._cancel_all_lighter()
                        await asyncio.sleep(0.05)
                        if self._fill_event.is_set():
                            self.state = State.HEDGING
                    ...
                    # 双向modify跟踪Extended
                    _c_min_move = self.extended_tick_size * 1
                    _close_need_mod = (abs(ext_bid - self._l_close_ext_bid_at_place) >= _c_min_move
                                       or abs(ext_ask - self._l_close_ext_ask_at_place) >= _c_min_move)
                    _c_since_mod = now - self._last_modify_ts
                    if _close_need_mod and _close_age >= 0.5 and _c_since_mod >= 0.5:
                        _new_price = self.compute_close_maker_price(ext_bid, ext_ask, l_bid, l_ask)
                        if self._l_close_order_idx and _new_price and _new_price != self._l_close_price:
                            ok = await self._modify_lighter_order(
                                self._l_close_order_idx, _new_price, self.order_size)
                            if ok:
                                _dir = "↑" if _new_price > self._l_close_price else "↓"
                                self._l_close_price = _new_price
                                self._l_close_ext_bid_at_place = ext_bid
                                self._l_close_ext_ask_at_place = ext_ask
                                self._last_modify_ts = time.time()
                                self.logger.info(f"[CLOSING] Modify {_dir} {_new_price}")
                        if self._fill_event.is_set():
                            continue
                ...
                # ━━━━━━ STATE: HEDGING ━━━━━━
                    ...
                    if _hedge_ok and _x_fill_price:
                        ...
                        _fee = float(FEE_EXTENDED_TAKER + FEE_LIGHTER_MAKER) * 10000
                        _net = _spread - _fee
                        _tag = "close" if self._is_close_cycle else "open"
                        self.cumulative_net_bps += _net
                        _dollar = _net / 10000 * float(_qty) * float(_x_fill_price)
                        self.cumulative_dollar_pnl += _dollar

                        _label = "[平仓成交]" if self._is_close_cycle else "[开仓成交]"
                        self.logger.info(
                            f"{_label} L_{_side}@{_l_price} → X_{_hedge_side}@{_x_fill_price}  "
                            f"spread={_spread:.2f}bps fee={_fee:.2f}bps net={_net:+.2f}bps  "
                            f"${_dollar:+.4f}  hedge={_hedge_ms:.0f}ms  "
                            f"累积=${self.cumulative_dollar_pnl:+.4f}")

                        self._trade_seq += 1
                        _tid = f"v26_{_tag}_{self._trade_seq:04d}"
                        _real_pnl_str, _cum_real_str, _bal_l_str, _bal_x_str = "", "", "", ""
                        if self._is_close_cycle:
                            _real_pnl_str, _cum_real_str, _bal_l_str, _bal_x_str = \
                                await self._v23_verify_close_pnl(
                                    _net, _dollar, _qty, Decimal(str(_x_fill_price)))
                        self._csv_row([
                            datetime.now(_TZ_CN).isoformat(),
                            _tid, _side,
                        ...
                        if self._is_close_cycle:
                            if self.ladder_step_bps > 0 and self._ladder_level > 0:
                                _old_lv = self._ladder_level
                                self._ladder_level = max(0, self._ladder_level - 1)
                                self.logger.info(f"[阶梯] 平仓1层, L{_old_lv} → L{self._ladder_level}")
...
def main():
    p = argparse.ArgumentParser(description="V26 阶梯建仓+逐层平仓")
