#!/usr/bin/env python3
"""
Extended + Lighter 价差扩张套利 V12（双边全市价 Taker-Taker）

策略（与 V11 收敛策略完全相反）：
  偏差检测: 运行时自动统计两所价差方向, 判断哪边常溢价/贴水
  开仓: 检测价差(taker执行价) ≤ entry_spread（价差很小/归零时入场）→
        按偏差方向: 溢价方做多 + 贴水方做空
  平仓: 检测价差扩张(taker执行价) ≥ target_spread →
        Lighter IOC 市价 + Extended IOC 市价 平仓（收割扩张利润）
  阶梯: 持仓中价差进一步收缩 → 追加仓位（降低平均入场价差）

逻辑:
  Lighter 和 Extended 经常表现出一方溢价一方贴水的稳定偏差.
  当价差临时收窄到接近 0 时入场, 等价差恢复到正常溢价/贴水水平时平仓.
  本质上是赌价差会回到历史常态方向.

费用: ~5.0 bps 往返 (Extended taker 2.5bps×2, Lighter taker 0% Premium)

与 V11 的区别：
  V11: 价差大时开仓, 等价差收敛平仓（做均值回归）
  V12: 价差小时开仓, 等价差扩张平仓（做方向性偏差回归）

用法:
  python extended_lighter_arb_v12.py --symbol BTC --size 0.002 --target-spread 6
  python extended_lighter_arb_v12.py --symbol ETH --dry-run
"""
...
class TakerBot:
    ...
    def __init__(
        ...
        # V12: Bias detection — track which exchange tends to be premium
        self._spread_samples: List[float] = []  # L_mid - X_mid in bps
        self._bias_window: int = 200  # rolling window size
        self._bias_direction: Optional[str] = None  # "L_premium" or "X_premium"
        self._bias_strength_bps: float = 0.0
        self._bias_min_samples: int = 20

        os.makedirs("logs", exist_ok=True)
        self.csv_path = f"logs/arb_v12_{self.symbol}_trades.csv"
        self.log_path = f"logs/arb_v12_{self.symbol}.log"
        self._init_csv()
    ...
    def _init_logger(self) -> logging.Logger:
        lg = logging.getLogger(f"arb_v12_{self.symbol}")
    ...
    # ═══════════════════════════════════════════════
    #  V12 Bias Detection + Spread Detection
    # ═══════════════════════════════════════════════

    def _update_bias(self, l_mid: Decimal, x_mid: Decimal):
        """Track rolling spread samples to detect which exchange is typically premium."""
        if l_mid <= 0 or x_mid <= 0:
            return
        avg = (l_mid + x_mid) / 2
        spread_bps = float((l_mid - x_mid) / avg * 10000)
        self._spread_samples.append(spread_bps)
        if len(self._spread_samples) > self._bias_window:
            self._spread_samples = self._spread_samples[-self._bias_window:]

        if len(self._spread_samples) >= self._bias_min_samples:
            avg_spread = sum(self._spread_samples) / len(self._spread_samples)
            self._bias_strength_bps = abs(avg_spread)
            if avg_spread > 0.5:
                self._bias_direction = "L_premium"
            elif avg_spread < -0.5:
                self._bias_direction = "X_premium"
            else:
    ...
    def _check_open_opportunity(
        ...
        """V12: Open when spread is SMALL and we have a bias direction.
        Entry when spread ≤ entry_spread (close_gap_bps).
        Direction determined by detected bias:
          L_premium → expect L to become more expensive → long L, short X
          X_premium → expect X to become more expensive → long X, short L
        """
        if self._bias_direction is None:
            return None
        ...
        if l_mid <= 0 or x_mid <= 0:
            return None
        avg = (l_mid + x_mid) / 2
        current_spread_bps = float((l_mid - x_mid) / avg * 10000)

        if abs(current_spread_bps) > self.close_gap_bps:
            return None

        if self._bias_direction == "L_premium":
            return ("long_L_short_X", "buy", "sell", l_ask, x_bid,
                    abs(current_spread_bps))
        else:
            return ("long_X_short_L", "sell", "buy", l_bid, x_ask,
                    abs(current_spread_bps))
    ...
    def _check_close_condition(
        ...
        """V12: Close when spread has EXPANDED past target in the bias direction."""
        if self.position_direction == "long_L_short_X":
            current_gap = l_bid - x_ask
            ref = (l_bid + x_ask) / 2
            gap_bps = float(current_gap / ref * 10000) if ref > 0 else 0.0
            return gap_bps >= self.target_spread
        elif self.position_direction == "long_X_short_L":
            current_gap = x_bid - l_ask
            ref = (x_bid + l_ask) / 2
            gap_bps = float(current_gap / ref * 10000) if ref > 0 else 0.0
            return gap_bps >= self.target_spread
        return False
    ...
    async def _open_position(
        ...
        async with self._trade_lock:
            ...
            if l_ok and x_ok:
                ...
                self._last_open_spread_bps = spread_bps
    ...
    async def _main_loop_wrapper(self):
        ...
        self.logger.info(
            f"启动 V12 价差扩张套利  {self.symbol}  "
            f"size={self.order_size} max_pos={self.max_position}  "
            f"入场价差≤{self.close_gap_bps}bps（价差收窄时入场）  "
            f"平仓价差≥{self.target_spread}bps（价差扩张时平仓）  "
        ...
        if not self.dry_run:
            ...
            if abs(self.lighter_position) > 0 or abs(self.extended_position) > 0:
                ...
                if x_est > 0 and l_est > 0:
                    ...
                    avg_est = (l_est + x_est) / 2
                    _est_sp = abs(float((l_est - x_est) / avg_est * 10000)) if avg_est > 0 else 0.0
                    self._last_open_spread_bps = _est_sp
            ...
            try:
                ...
                if now - last_status_log > 15:
                    ...
                    if (self.state == State.IN_POSITION
                        ...
                        if self.position_direction == "long_L_short_X":
                            _cur_gap = l_bid - x_ws_ask
                            _ref = (l_bid + x_ws_ask) / 2
                        else:
                            _cur_gap = x_ws_bid - l_ask
                            _ref = (x_ws_bid + l_ask) / 2
                        _gap_bps = float(_cur_gap / _ref * 10000) if _ref > 0 else 0.0
                        _close_ok = _gap_bps >= self.target_spread
                        ...
                        basis_info = (
                            f" gap={_gap_bps:+.1f}bps/≥{self.target_spread}bps"
                            f"({'CLOSE' if _close_ok else 'HOLD'}"
                            f" entry={_open_sp_bps:.1f}bps hold={_hold:.0f}s)")
                    bias_info = ""
                    if self._bias_direction:
                        bias_info = (f"  偏差={self._bias_direction}"
                                     f"({self._bias_strength_bps:.1f}bps"
                                     f"/{len(self._spread_samples)}样本)")
                    ladder_info = ""
                    if (self.ladder_step > 0
                        ...
                        _next = max(0, self._last_open_spread_bps - self.ladder_step)
                        ladder_info = (
                    ...
                    self.logger.info(
                        ...
                        f"{basis_info}{ladder_info}{bias_info}  "
                ...
                if self.state == State.IDLE:
                    ...
                    if opp and not self.dry_run:
                        ...
                        self.logger.info(
                            f"[价差收窄入场] {direction} "
                            f"当前价差={sp_bps:.1f}bps≤{self.close_gap_bps}bps "
                            f"偏差={self._bias_direction}({self._bias_strength_bps:.1f}bps) "
                        ...
                        if now - last_status_log > 5:
                            self.logger.info(
                                f"[DRY-RUN 价差收窄] {direction} "
                                f"价差={sp_bps:.1f}bps "
                                f"偏差={self._bias_direction}")
                ...
                # ━━━━━━ STATE: IN_POSITION ━━━━━━
                    ...
                    if (x_ws_bid is not None and x_ws_ask is not None
                        ...
                        # V12 Ladder: add layer when spread contracts further
                        ladder_opened = False
                        if (self.ladder_step > 0
                            ...
                            if hold_elapsed <= self.max_hold_sec * 0.7:
                                avg = (l_mid + x_mid) / 2
                                current_abs_spread = abs(float(
                                    (l_mid - x_mid) / avg * 10000
                                )) if avg > 0 else 999.0
                                ladder_threshold = max(
                                    0, self._last_open_spread_bps - self.ladder_step)
                                if current_abs_spread <= ladder_threshold:
                                    self._last_ladder_attempt = now
                                    if current_abs_spread <= self.close_gap_bps:
                                        cur_layers = int(
                                        ...
                                        if self.position_direction == "long_L_short_X":
                                            ...
                                            l_ref, x_ref = l_ask, x_ws_bid
                                        else:
                                            l_side, x_side = "sell", "buy"
                                            l_ref, x_ref = l_bid, x_ws_ask
                                        self.logger.info(
                                            f"[阶梯] 价差收窄加仓 {cur_layers}"
                                            f"/{max_layers} "
                                            f"spread={current_abs_spread:.1f}bps")
                                        ok = await self._open_position(
...
def main():
    p = argparse.ArgumentParser(
        description="V12 价差扩张套利 (Lighter+Extended)")
    ...
    p.add_argument("--target-spread", type=float, default=6.0,
                   help="平仓目标: 价差扩张到≥此值(bps)即平仓")
    p.add_argument("--close-gap-bps", type=float, default=2.0,
                   help="入场阈值: 价差收窄到≤此值(bps)时入场")
    ...
    p.add_argument("--ladder-step", type=float, default=0.0,
                   help="阶梯加仓: 价差每收窄此值(bps)追加一层")
    p.add_argument("--ladder-cooldown", type=float, default=10.0,
                   help="阶梯加仓最小间隔(秒), 防止短时间内连续加仓")
    p.add_argument("--bias-window", type=int, default=200,
                   help="偏差检测滚动窗口(采样次数)")
    p.add_argument("--bias-min-samples", type=int, default=20,
                   help="偏差方向确认所需最少样本数")
    ...
    bot._bias_window = args.bias_window
    bot._bias_min_samples = args.bias_min_samples
