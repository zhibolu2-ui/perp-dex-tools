#!/usr/bin/env python3
"""
Extended + Lighter 跨所套利 V19 — Stale Quote Sniper

利用 Extended 报价滞后于 Lighter 的微观结构进行高频套利。
支持两种模式 (--mode A|B):

  Mode A (推荐): Extended Maker + Lighter Taker
    信号: Lighter价格偏离 Extended > threshold
    执行: 在 Extended BBO 挂POST_ONLY → 成交 → 立即Lighter taker对冲
    费用: Ext maker(0) + L Premium taker(0.4) = 0.4bps
    优势: 超低费用, 利用Extended过时报价

  Mode B: Lighter Maker + Extended Taker
    信号: 计算对冲edge (Extended BBO - safety)
    执行: 在 Lighter 挂POST_ONLY → 成交 → 立即Extended IOC对冲
    费用: L Premium maker(0.4) + Ext taker(2.5) = 2.9bps
    优势: Lighter成交活跃, 填充率更高

关键改进 (vs V18):
  - 在maker交易所自身BBO挂单, 不再按Lighter定价推远
  - 信号驱动: 只在偏离确认时挂单, 不盲目双向挂
  - Binance确认: 防止flash spike期间误挂
  - 短超时: 3-5s未成交即撤, 不浪费队列位置

用法:
  python extended_lighter_arb_v19.py --symbol BTC --mode A --size 0.001 \\
      --threshold 0.8 --fill-timeout 5
  python extended_lighter_arb_v19.py --symbol ETH --mode B --size 0.1 \\
      --threshold 3.5 --fill-timeout 15
"""
...
class TakerBot:
    ...
    # Per-leg exchange fees (bps, one-way) — Lighter Premium account
    FEE_EXTENDED_TAKER: float = 2.5
    FEE_EXTENDED_MAKER: float = 0.0
    FEE_LIGHTER_TAKER: float = 0.4   # Premium account
    FEE_LIGHTER_MAKER: float = 0.4   # Premium account

    # V19 mode pair fees
    PAIR_FEE_A: float = 0.4    # Mode A: Ext maker(0) + L taker(0.4)
    PAIR_FEE_B: float = 2.9    # Mode B: L maker(0.4) + Ext taker(2.5)

    # Legacy pair fees (kept for fallback / taker paths)
    PAIR_FEE_TT: float = 2.9     # X taker(2.5) + L taker(0.4) — Premium
    PAIR_FEE_MT: float = 0.4     # X maker(0) + L taker(0.4) — same as Mode A
    PAIR_FEE_MM: float = 0.4     # X maker(0) + L maker(0.4) — both maker
    ...
    def __init__(
        ...
        # ── V19 Stale Quote Sniper ──
        self.strategy_mode: str = "A"   # "A" = Ext maker + L taker; "B" = L maker + Ext taker
        self._v19_threshold_bps: float = 0.8
        self._v19_fill_timeout: float = 5.0
        self._v19_bn_confirm: bool = True
        self._v19_quoting_side: Optional[str] = None
        self._v19_quoting_order_id: Optional[str] = None
        self._v19_quoting_price: Decimal = Decimal("0")
        self._v19_quoting_start: float = 0.0
        self._v19_quoting_qty: Decimal = Decimal("0")
        self._v19_hedge_retries: int = 0
        self._v19_fill_price: Optional[Decimal] = None
        self._v19_fill_qty: Decimal = Decimal("0")
        self._v19_consecutive_signals: int = 0
        ...
        self.csv_path = f"logs/arb_v19_{self.symbol}_trades.csv"
        self.log_path = f"logs/arb_v19_{self.symbol}.log"
        self._init_csv()
    ...
    def _init_logger(self) -> logging.Logger:
        lg = logging.getLogger(f"arb_v19_{self.symbol}")
    ...
    def _write_csv_row(self, d: dict):
        """V19 convenience wrapper: write a trade row from a dict."""
        bn_vol = d.get("bn_vol_bps", "0")
        row = [
            d.get("ts", ""), "V19_ROUND_TRIP", d.get("route", ""),
            "", "", d.get("l_price", ""), d.get("qty", ""),
            d.get("x_side", ""), "", d.get("x_price", ""), d.get("qty", ""),
            d.get("spread_bps", ""), d.get("hedge_latency_ms", ""),
            f"V19_{d.get('route', '')}",
            "", "", "", "",
            d.get("net_bps", ""), d.get("cumulative_bps", ""),
            d.get("trade_id", ""), d.get("route", ""),
            "0", d.get("total_fee_bps", ""), d.get("total_fee_bps", ""),
            d.get("hedge_latency_ms", ""), "N",
            d.get("x_side", ""), bn_vol,
            "", "",
        ]
        self._csv_row(row)
    ...
    # ═══════════════════════════════════════════════
    #  V19 Stale Quote Signal + Pricing
    # ═══════════════════════════════════════════════

    def _v19_check_divergence(
        self, l_bid: Decimal, l_ask: Decimal,
        x_bid: Decimal, x_ask: Decimal,
    ) -> Optional[Tuple[str, float]]:
        """Check for cross-exchange divergence.
        Returns (side, divergence_bps) or None.
        side='buy' means buy on Extended, sell on Lighter.
        side='sell' means sell on Extended, buy on Lighter.
        """
        mid = (l_bid + l_ask + x_bid + x_ask) / 4
        if mid <= 0:
            return None

        long_gap = l_bid - x_ask
        long_bps = float(long_gap / mid * 10000)

        short_gap = x_bid - l_ask
        short_bps = float(short_gap / mid * 10000)

        threshold = self._v19_threshold_bps

        if long_bps >= threshold and long_bps >= short_bps:
            return ("buy", long_bps)
        elif short_bps >= threshold:
            return ("sell", short_bps)
        return None

    def _v19_bn_confirms(self, side: str, l_mid: float) -> bool:
        """Check if Binance mid agrees with the divergence direction."""
        if not self._v19_bn_confirm:
            return True
        bf = self.binance_feed
        if not bf or not bf.connected or bf.mid <= 0:
            return True
        bn_mid = bf.mid
        bn_l_gap_bps = (bn_mid - l_mid) / l_mid * 10000
        if side == "buy":
            return bn_l_gap_bps > -2.0
        else:
            return bn_l_gap_bps < 2.0

    def _v19_price_mode_a(
        self, side: str, x_bid: Decimal, x_ask: Decimal,
    ) -> Decimal:
        """Mode A: price at Extended's own BBO."""
        tick = self.extended_tick_size
        if side == "buy":
            return x_ask - tick
        else:
    ...
    def _v19_price_mode_b(
        self, side: str,
        l_bid: Decimal, l_ask: Decimal,
        x_bid: Decimal, x_ask: Decimal,
    ) -> Optional[Decimal]:
        """Mode B: price Lighter maker relative to Extended BBO.
        Returns price for Lighter POST_ONLY order, or None if no valid price.
        """
        tick = self.lighter_tick
        fee_bps = Decimal(str(self.PAIR_FEE_B)) / Decimal("10000")

        if side == "buy":
            target = x_bid * (Decimal("1") - fee_bps)
            price = min(target, l_ask - tick)
            if price <= Decimal("0") or price >= l_ask:
                return None
            return self._round_lighter_price(price)
        else:
            target = x_ask * (Decimal("1") + fee_bps)
            price = max(target, l_bid + tick)
            if price <= l_bid:
                return None
            return self._round_lighter_price(price)

    def _v19_reset_quoting(self):
        """Reset quoting state."""
        self._v19_quoting_side = None
        self._v19_quoting_order_id = None
        self._v19_quoting_price = Decimal("0")
        self._v19_quoting_start = 0.0
        self._v19_quoting_qty = Decimal("0")
        self._v19_hedge_retries = 0
        self._v19_fill_price = None
        self._v19_fill_qty = Decimal("0")

    # ═══════════════════════════════════════════════
    #  Spread Detection (legacy V18)
    # ═══════════════════════════════════════════════
    ...
    async def _main_loop_wrapper(self):
        _mode_name = "A (Ext Maker + L Taker)" if self.strategy_mode == "A" else "B (L Maker + Ext Taker)"
        _pair_fee = self.PAIR_FEE_A if self.strategy_mode == "A" else self.PAIR_FEE_B
        self.logger.info(
            f"启动 V19 Stale Quote Sniper  Mode {_mode_name}  {self.symbol}  "
            f"size={self.order_size} max_pos={self.max_position}  "
            f"threshold={self._v19_threshold_bps}bps  "
            f"fill_timeout={self._v19_fill_timeout}s  "
            f"pair_fee={_pair_fee:.1f}bps  "
            f"bn_confirm={self._v19_bn_confirm}  "
        ...
        if not self.dry_run:
            ...
            if abs(self.lighter_position) > 0 or abs(self.extended_position) > 0:
                self.logger.warning(
                    ...
                    f"X={self.extended_position}, V19紧急平仓")
                await self._emergency_flatten()
                await self._reconcile_positions()
        ...
        while not self.stop_flag:
            try:
                ...
                if now - last_status_log > 15:
                    dyn_thr = self._v19_threshold_bps
                    hold_s = ""
                    ...
                    v19_info = f"  thr={self._v19_threshold_bps:.1f}bps"
                    if x_ws_bid and x_ws_ask:
                        _sig = self._v19_check_divergence(
                            l_bid, l_ask, x_ws_bid, x_ws_ask)
                        if _sig:
                            v19_info += f" sig={_sig[0]}/{_sig[1]:.1f}bps"
                    if self.state == State.QUOTING:
                        _qa = now - self._v19_quoting_start
                        v19_info += (
                            f" QUOTING:{self._v19_quoting_side}"
                            f"@{self._v19_quoting_price}/{_qa:.1f}s")
                    maker_info = v19_info
                    ladder_info = ""
                ...
                # ── Force reconcile (skip during active quoting/hedging) ──
                if self._force_reconcile and not self.dry_run:
                    if self.state in (State.QUOTING, State.HEDGING):
                        self.logger.info(
                            "[强制对账] 活跃交易中, 延后对账")
                    else:
                ...
                # ── Periodic reconcile (skip during active quoting/hedging) ──
                ...
                if self.state == State.IDLE:
                    ...
                    signal = self._v19_check_divergence(
                        l_bid, l_ask, x_ws_bid, x_ws_ask)
                    if signal is None:
                        await self._interruptible_sleep(self.interval)
                        continue

                    _sig_side, _sig_bps = signal
                    _l_mid_f = float(l_mid)

                    if not self._v19_bn_confirms(_sig_side, _l_mid_f):
                        self.logger.debug(
                            f"[V19] {_sig_side} div={_sig_bps:.1f}bps "
                            f"但BN不确认, 跳过")
                        await self._interruptible_sleep(self.interval)
                        continue

                    if self.strategy_mode == "A":
                        _price = self._v19_price_mode_a(
                            _sig_side, x_ws_bid, x_ws_ask)
                        _price = _price.quantize(
                            self.extended_tick_size, rounding=ROUND_HALF_UP)
                        self.logger.info(
                            f"[V19-A] 信号={_sig_side} div={_sig_bps:.1f}bps "
                            f"→ Extended POST_ONLY {_sig_side} "
                            f"@ {_price} (X={x_ws_bid}/{x_ws_ask})")

                        oid = await self._place_extended_post_only(
                            _sig_side, self.order_size, _price)
                        if oid:
                            self._v19_quoting_side = _sig_side
                            self._v19_quoting_order_id = oid
                            self._v19_quoting_price = _price
                            self._v19_quoting_start = time.time()
                            self._v19_quoting_qty = self.order_size
                            self._bn_mid_at_place = (
                                self.binance_feed.mid
                                if self.binance_feed and self.binance_feed.connected
                                else 0)
                            self.state = State.QUOTING
                        else:
                            self.logger.warning("[V19-A] 挂单失败")
                            await asyncio.sleep(1.0)

                    elif self.strategy_mode == "B":
                        _price = self._v19_price_mode_b(
                            _sig_side, l_bid, l_ask, x_ws_bid, x_ws_ask)
                        if _price is None:
                            await self._interruptible_sleep(self.interval)
                            continue
                        self.logger.info(
                            f"[V19-B] 信号={_sig_side} div={_sig_bps:.1f}bps "
                            f"→ Lighter POST_ONLY {_sig_side} "
                            f"@ {_price} (L={l_bid}/{l_ask})")

                        oid = await self._place_lighter_maker(
                            _sig_side, self.order_size, _price)
                        if oid:
                            self._v19_quoting_side = _sig_side
                            self._v19_quoting_order_id = str(oid)
                            self._v19_quoting_price = _price
                            self._v19_quoting_start = time.time()
                            self._v19_quoting_qty = self.order_size
                            self.state = State.QUOTING
                        else:
                            self.logger.warning("[V19-B] 挂单失败")
                            await asyncio.sleep(1.0)

                # ━━━━━━ STATE: QUOTING (V19) ━━━━━━
                elif self.state == State.QUOTING:
                    _q_age = now - self._v19_quoting_start
                    _q_side = self._v19_quoting_side

                    if self.strategy_mode == "A":
                        if self._x_maker_confirmed.is_set():
                            _fill_qty = self._x_maker_fill_qty
                            _fill_price = self._x_maker_fill_price
                            if _fill_qty > 0 and _fill_price:
                                self.logger.info(
                                    f"[V19-A] Extended {_q_side} FILLED "
                                    f"{_fill_qty} @ {_fill_price} "
                                    f"({_q_age*1000:.0f}ms)")
                                self._v19_fill_price = _fill_price
                                self._v19_fill_qty = _fill_qty
                                self.state = State.HEDGING
                                continue
                            else:
                                self.logger.warning(
                                    f"[V19-A] 订单结束但无成交, 重置")
                                self._v19_reset_quoting()
                                self.state = State.IDLE
                                continue

                        if _q_age > self._v19_fill_timeout:
                            self.logger.info(
                                f"[V19-A] 超时({_q_age:.1f}s), 撤单")
                            _oid = self._x_maker_pending_id
                            if _oid:
                                await self._cancel_extended_order(_oid)
                            await asyncio.sleep(0.1)
                            if (self._x_maker_confirmed.is_set()
                                    and self._x_maker_fill_qty > 0):
                                continue
                            self._v19_reset_quoting()
                            self.state = State.IDLE
                            continue
                        ...
                        if self._lighter_fill_event.is_set():
                            _fd = self._lighter_fill_data
                            self._lighter_fill_event.clear()
                            if _fd:
                                _fill_qty = Decimal(str(
                                    _fd.get("filled_size", "0")))
                                _fill_price = Decimal(str(
                                    _fd.get("avg_price", "0")))
                                if _fill_qty > 0 and _fill_price > 0:
                                    self.logger.info(
                                        f"[V19-B] Lighter {_q_side} FILLED "
                                        f"{_fill_qty} @ {_fill_price} "
                                        f"({_q_age*1000:.0f}ms)")
                                    self._v19_fill_price = _fill_price
                                    self._v19_fill_qty = _fill_qty
                        ...
                        if _q_age > self._v19_fill_timeout:
                            self.logger.info(
                                f"[V19-B] 超时({_q_age:.1f}s), 撤单")
                            await self._do_lighter_cancel_all()
                            self._v19_reset_quoting()
                            self.state = State.IDLE
                            continue

                        if _q_age > 2.0 and x_ws_bid and x_ws_ask:
                            _new_sig = self._v19_check_divergence(
                                l_bid, l_ask, x_ws_bid, x_ws_ask)
                            if _new_sig is None:
                                self.logger.info(
                                    f"[V19-B] 偏离已消失, 撤单")
                ...
                # ━━━━━━ STATE: HEDGING (V19) ━━━━━━
                    ...
                    _h_side = self._v19_quoting_side
                    _h_qty = self._v19_fill_qty
                    _h_fill_price = self._v19_fill_price
                    _hedge_side = "sell" if _h_side == "buy" else "buy"
                    _hedge_start = time.time()

                    # 先更新 maker 侧仓位（WS callback 可能没及时更新）
                    if self.strategy_mode == "A":
                        if _h_side == "buy":
                            self.extended_position += _h_qty
                        else:
                            self.extended_position -= _h_qty

                        self.logger.info(
                            f"[V19-A] 对冲: Lighter taker {_hedge_side} "
                            f"{_h_qty} (X仓位已更新={self.extended_position})")
                        _hedge_ok = False
                        for _retry in range(3):
                            _filled_qty = await self._place_lighter_taker(
                                _hedge_side, _h_qty,
                                _h_fill_price if _h_fill_price else l_mid)
                            if _filled_qty is not None and _filled_qty > 0:
                                _hedge_ok = True
                                _l_avg = self._last_lighter_avg_price
                                if _l_avg is None or _l_avg <= 0:
                                    _l_avg = _h_fill_price
                                _hedge_ms = (time.time() - _hedge_start) * 1000
                                _pair_fee = self.PAIR_FEE_A
                                if _h_side == "buy":
                                    _spread_bps = float(
                                        (_l_avg - _h_fill_price)
                                        / _h_fill_price * 10000
                                    ) if _h_fill_price and _h_fill_price > 0 else 0
                                else:
                                    _spread_bps = float(
                                        (_h_fill_price - _l_avg)
                                        / _l_avg * 10000
                                    ) if _l_avg and _l_avg > 0 else 0
                                _net_bps = _spread_bps - _pair_fee
                                self.cumulative_net_bps += _net_bps
                                self.logger.info(
                                    f"[V19-A] 对冲成功! "
                                    f"X_{_h_side}@{_h_fill_price} → "
                                    f"L_{_hedge_side}@{_l_avg} "
                                    f"spread={_spread_bps:.2f}bps "
                                    f"fee={_pair_fee:.1f}bps "
                                    f"net={_net_bps:+.2f}bps "
                                    f"hedge={_hedge_ms:.0f}ms "
                                    f"累积={self.cumulative_net_bps:+.2f}bps")
                                self._trade_seq += 1
                                self._write_csv_row({
                                    "ts": datetime.now().isoformat(),
                                    "type": "ROUND_TRIP",
                                    "trade_id": f"v19_{self._trade_seq}",
                                    "route": f"A_{_h_side}",
                                    "x_side": _h_side,
                                    "x_price": str(_h_fill_price),
                                    "l_price": str(_l_avg),
                                    "qty": str(_h_qty),
                                    "spread_bps": f"{_spread_bps:.2f}",
                                    "total_fee_bps": f"{_pair_fee:.1f}",
                                    "net_bps": f"{_net_bps:.2f}",
                                    "hedge_latency_ms": f"{_hedge_ms:.0f}",
                                    "bn_vol_bps": f"{self.binance_feed.realized_vol_bps:.1f}" if self.binance_feed else "0",
                                    "cumulative_bps": f"{self.cumulative_net_bps:.2f}",
                                })
                                self._consecutive_hedge_failures = 0
                                break
                            else:
                                self.logger.warning(
                                    f"[V19-A] 对冲重试 {_retry+1}/3")
                                await asyncio.sleep(0.5)
                        ...
                        if not _hedge_ok:
                            ...
                            if self._consecutive_hedge_failures >= self._max_hedge_failures:
                                self._hedge_circuit_breaker_until = (
                                    time.time() + self._hedge_circuit_pause)
                                self.logger.error(
                                    f"[V19] 熔断! 暂停{self._hedge_circuit_pause}s")
                            await self._reconcile_positions()
                            if (abs(self.lighter_position) > 0
                                    or abs(self.extended_position) > 0):
                                await self._emergency_flatten()
                            self._reset_position_state()

                    elif self.strategy_mode == "B":
                        if _h_side == "buy":
                            self.lighter_position += _h_qty
                        else:
                            self.lighter_position -= _h_qty

                        self.logger.info(
                            f"[V19-B] 对冲: Extended IOC {_hedge_side} "
                            f"{_h_qty} (L仓位已更新={self.lighter_position})")
                        _hedge_ok = False
                        for _retry in range(3):
                            _filled_qty = await self._place_extended_ioc(
                                _hedge_side, _h_qty,
                                _h_fill_price if _h_fill_price else x_mid)
                            if _filled_qty is not None and _filled_qty > 0:
                                _hedge_ok = True
                                _x_avg = self._x_ioc_fill_price
                                if _x_avg is None or _x_avg <= 0:
                                    _x_avg = _h_fill_price
                                _hedge_ms = (time.time() - _hedge_start) * 1000
                                _pair_fee = self.PAIR_FEE_B
                                if _h_side == "buy":
                                    _spread_bps = float(
                                        (_h_fill_price - _x_avg)
                                        / _x_avg * 10000
                                    ) if _x_avg and _x_avg > 0 else 0
                                else:
                                    _spread_bps = float(
                                        (_x_avg - _h_fill_price)
                                        / _h_fill_price * 10000
                                ...
                                self.logger.info(
                                    f"[V19-B] 对冲成功! "
                                    f"L_{_h_side}@{_h_fill_price} → "
                                    f"X_{_hedge_side}@{_x_avg} "
                                    f"spread={_spread_bps:.2f}bps "
                                    f"fee={_pair_fee:.1f}bps "
                                    f"net={_net_bps:+.2f}bps "
                                    f"hedge={_hedge_ms:.0f}ms "
...
def main():
    p = argparse.ArgumentParser(
        description="V19 Stale Quote Sniper (Extended+Lighter)")
    p.add_argument("-s", "--symbol", default="BTC")
    p.add_argument("--mode", choices=["A", "B"], default="A",
                   help="A=Ext maker+L taker (0.4bps) | B=L maker+Ext taker (2.9bps)")
    p.add_argument("--size", type=str, default="0.001")
    p.add_argument("--max-position", type=str, default="0.005")
    p.add_argument("--threshold", type=float, default=0.8,
                   help="偏离阈值bps (Mode A推荐0.5-1.0, Mode B推荐3.0-4.0)")
    p.add_argument("--fill-timeout", type=float, default=5.0,
                   help="挂单等待超时秒数 (Mode A: 3-5s, Mode B: 10-30s)")
    p.add_argument("--no-bn-confirm", action="store_true",
                   help="禁用Binance方向确认")
    ...
    p.add_argument("--flatten-on-start", action="store_true",
                   help="启动时检测到遗留仓位自动紧急平仓")
    p.add_argument("--dry-run", action="store_true")
    p.add_argument("--env-file", default=".env")
    p.add_argument("--target-spread", type=float, default=1.0)
    p.add_argument("--close-gap-bps", type=float, default=0.5)
    ...
    bot.strategy_mode = args.mode
    bot._v19_threshold_bps = args.threshold
    bot._v19_fill_timeout = args.fill_timeout
    bot._v19_bn_confirm = not args.no_bn_confirm
    bot._max_slippage_bps = args.max_slippage_bps
    bot.flatten_on_start = args.flatten_on_start
    bot.maker_mode = True

    if args.mode == "A":
        bot.ROUND_TRIP_FEE_BPS = bot.PAIR_FEE_A
    else:
        bot.ROUND_TRIP_FEE_BPS = bot.PAIR_FEE_B

    _mode_name = "A (Ext Maker + L Taker)" if args.mode == "A" else "B (L Maker + Ext Taker)"
    _pair_fee = bot.PAIR_FEE_A if args.mode == "A" else bot.PAIR_FEE_B
    print(f"[V19] ★ Stale Quote Sniper — Mode {_mode_name}")
    print(f"  {args.symbol} size={args.size} max_pos={args.max_position}")
    print(f"  threshold={args.threshold}bps  fill_timeout={args.fill_timeout}s")
    print(f"  pair_fee={_pair_fee:.1f}bps  bn_confirm={bot._v19_bn_confirm}")
